...
| N20-lore-eval-MCP | feat | **构建侧 CC 驱动 eval MCP（build-mcp）缺失**：F3 已补 lore offline mock harness（零 LLM），但缺「CC 当作者驱动真构建 GM」的对称路径——跑团侧已有 `dicelore-play` MCP（CC 经它连真后端当玩家驱动真 GM），构建侧无对应入口，真 LLM 一局无从跑。建 `apps/orchestrator/eval/build-mcp.ts`（对称 dice `play-mcp.ts`，连 build HTTP/LoreSession，CC 当作者驱动真构建 GM）+ 注册 `dicelore-build` 进 `.mcp.json` + `.claude/settings.local.json` enabledMcpjsonServers，让**跑团 + 构建两侧都能 CC 驱动 eval**。是 F3 offline mock 的 CC 驱动对称路径，喂 F3「真 LLM 一局」 | 用户 2026-06-26 指示 | ✓（lore 侧真 LLM eval 无入口） | 🆕 待开发。**时序：第三批剩余未完成推完后做**。依赖 F3 offline harness（✅）+ build HTTP/LoreSession（✅ RT-5 后 REST 语义） |
