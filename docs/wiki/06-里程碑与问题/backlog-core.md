...
>
> **🔑 2026-06-29 方法论决策（待写 ADR，将修订 [ADR-0025](../05-决策记录-ADR/README.md)）**：eval 的**对照系 = `docs/research/` 真实案例**（兽人冒险/抽卡/恶龙团→喂进团本构建库、和 lore GM 对话建出团本→人工认可→Claude skill+MCP 拿团本跑 eval→落盘 eval 报告）。**废 doctrine-vs-baseline A/B 对照**——不再以"去教条档"作教条价值的证据系。**量化以目前认知不可行 → 闭环产出定性报告**（量化口径待探索）。波及 ADR-0025 + wiki 01 成功标准 / 04 Skills-eval·Skills包·团本构建工具链 + `eval/grader.md` + `dicelore-eval` skill，**走决策→ADR→传播**（ADR 定稿后由它单源传播，下方 baseline 欠账项一并作废）。
...
| F3 | feat | **团本构建(lore)侧零 eval**：现 eval harness 只覆盖跑团(dice)侧（play-mcp 连真后端当玩家跑剧本），构建团本侧（LoreSession/构建 GM/组件5·6 import/构建工具面）无任何评估。后果：构建流程「作者↔构建 GM」交互质量、import 映射正确性、构建工具可用性**全无客观验证**——与 play 侧 F1 同等不可信 | 用户指正 | ✓（随构建能力扩展越痛） | 🟡 offline 部分 ✅ 达成（2026-06-26）：`src/eval/loreScenario.ts`+`loreAssertions.ts`+`eval/loreRun.ts`+`loreScenarios/frontier-saga.json`+`loreGrader.md`——确定性 mock 作者 `buildCalls`→`runBuildTool`→commit→`importPack`→`gradeLoreRun` 判运行库（零 LLM，7 测）。**真构建 GM 真 LLM 一局留「待测试」**——CC 驱动对称路径见 [N20-lore-eval-MCP](#)（本页主题F 新增）。终局场景 eval 随后补（依赖 [E2 ADR-0026](../05-决策记录-ADR/README.md) 定稿落地） |
...
| # | 类型 | 问题 | 来源 | 恶化 | 下一步/依赖 |
|---|------|------|------|:--:|--------|
| F1-floor | feat | **F1 机械地板缺失**（QA-002）：`assertions` 只统计 verdict 工具调用次数，无"该骰却没骰"检测；`grade.ts` L60 注释的 F1 时序校验是 TODO 非实现。GM 不调 `resolve_*` 纯 narrate 编结果时 `verdictCount=0` 不会被标记 fail——eval 可被"不骰纯编"欺骗，F1 假性 pass | 2026-06-25 全量体检 | ✓（随 eval 作发版依据越痛） | ✅ 已达成（2026-06-26）：`eval/assertions.ts` 加 `rollFloor`——F1 掷骰绕过机械地板：① 计数地板（verdict 数 < `scenario.expects.minVerdicts` → fail，散文绕过/漏掷）② 时序地板（narrate 抢在所有 verdict 之前 = 先编结果后补掷 → fail）。`scenario.ts` 加 `expects.minVerdicts` 字段、4 场景声明、`eval/grade.ts` 接线。零 LLM、offline 可判 |
| F2-floor | feat | **F2 机械地板 + baseline 对照缺失**（QA-003）：F2 软着陆无机械检测全靠 grader LLM 判；`reports/` 仅 doctrine 报告无 baseline 对照——验收口径"with 应显著低于 baseline"无数据支撑；grader 自校准风险（讨好本能）在 harness 无实现 | 2026-06-25 全量体检 | ✓（同上） | ✅ 已达成（2026-06-26）：`eval/assertions.ts` 加 `closureFloor`——F2 软着陆**弱**地板：floor 恒 `advisory`（绝不硬判 pass/fail），检 is_moment / 收束参考词 / game_time 推进任一即 signalPresent，缺信号给 advisory 提示评测者重点核 F2。`scenario.expects.closure` 声明。⚠️ **原"baseline 对照报告"欠账作废**（2026-06-29 决策废 doctrine-vs-baseline A/B，见主题F 决策注记）→ 改为**对照 `docs/research` 真实案例的定性 eval** |
| F-harness-test | fix | **eval harness 自身测试覆盖薄**（QA-006）：`play-mcp.test.ts` 只测 open→start→send 闭环，`doChoose`/`doRoll`/`doBrowse`（eval 驱动 GM 跑完一局的关键动作）无任何测试调用；`list_scenarios` 无测试验场景清单非空/符合 schema；`assertions.test.ts` 不测 `grade.ts` 端到端评分流程 | 2026-06-25 全量体检 | ✓ | ✅ 已达成（2026-06-26）：新建 `src/eval/harness.test.ts`（15 用例）覆盖 offline 驱动 runTool 的 roll/choose/browse 等价路径 + 错误路径 + 枚举；`assertions.test.ts` 扩 F1/F2 正反例。⚠️ **实现实证/漂移**：backlog/路线图旧述提的 `src/eval/play-mcp.ts` 在 core 包**不存在**——真 harness 驱动是 `packages/core/eval/{run,grade,batch,tool}.ts`（offline `runTool` 路径，非 MCP），scenarios 在 `packages/core/eval/scenarios/`；`choose/roll/browse/list_scenarios` 是独立 **dicelore-play MCP server** 接口（另一包），不在 core eval 内。F-harness-test 已落到 in-scope 的 `src/eval/harness.test.ts` 覆盖等价 offline 驱动路径 |
| N20-lore-eval-MCP | feat | **构建侧 CC 驱动 eval MCP（build-mcp）缺失**：F3 已补 lore offline mock harness（零 LLM），但缺「CC 当作者驱动真构建 GM」的对称路径——跑团侧已有 `dicelore-play` MCP（CC 经它连真后端当玩家驱动真 GM），构建侧无对应入口，真 LLM 一局无从跑。建 `apps/orchestrator/eval/build-mcp.ts`（对称 dice `play-mcp.ts`，连 build HTTP/LoreSession，CC 当作者驱动真构建 GM）+ 注册 `dicelore-build` 进 `.mcp.json` + `.claude/settings.local.json` enabledMcpjsonServers，让**跑团 + 构建两侧都能 CC 驱动 eval**。是 F3 offline mock 的 CC 驱动对称路径，喂 F3「真 LLM 一局」 | 用户 2026-06-26 指示 | ✓（lore 侧真 LLM eval 无入口） | ✅ 达成（2026-06-26）：`apps/orchestrator/eval/build-mcp.ts`（对称 dice `play-mcp.ts`，CC 当作者经 stdio MCP 连构建 HTTP 驱动构建 GM）暴露 5 工具 `open_build_session`/`send_to_builder`/`get_draft`/`list_catalog`/`get_pack_files`。因 RT-5 后 lore 是 **REST only、构建 GM 响应不经 REST 返回**，eval 模型=**作者发指令 + 检视产物**（catalog/draft）而非收 GM 散文。补 additive `GET /lore-sessions/:id/draft`（复用 `getLoreSession`+`Draft.toPackFiles`/snapshot，填「构建中途 Draft 检视」gap，不改既有端点）。集成测起真 lore 后端 + fake 构建 agent 直驱 `dicelore_build_*` 工具，不烧 LLM（orchestrator 129 测）。已注册 `.mcp.json` + `settings.local.json` enabledMcpjsonServers `dicelore-build`，跑团(dicelore-play)+构建两侧均 CC 可驱动 eval。**真 LLM 构建 GM 一局留「待测试」**（同 F3 真 LLM 部分、dogfooding 真 LLM 一局）。浮现 follow-up [BE-checkout-head](backlog-后端.md#主题--团本构建组件56-)（fix·P3） |
...
| # | 类型 | 问题 | 来源 | 恶化 | 下一步/依赖 |
|---|------|------|------|:--:|--------|
| TB-1 | feat | **并发/超时/重启恢复/注入四类边界测试全缺**（CROSS-BOUNDARY-TEST，P1）：grep concurren/race/parallel across test files 零命中——BE-002 并发竞态无测试；grep timeout/abort/recover across .test.ts（非 live）零命中——BE-001 超时半途状态无测试；recovery.test.ts 测了 restagePendingRolls 重弹正向路径但未测"waiters 空 + POST /roll 409 死锁"场景；grep inject/ssrf/恶意/注入 across test files 零命中——ARCH-009/BE-005/BE-006 注入无测试 | 2026-06-25 全量体检 | ✓✓（随代码增长回归风险线性升） | ✅ 已达成（2026-06-26，wave3）：新增 `apps/orchestrator/src/dice/boundary.test.ts`（15 用例），四类边界全覆盖——① **并发**：HTTP 409 `turn_in_progress` 映射 + inflight 释放后可再触发；② **超时**：当前行为**基线**（RT-1 未修——GM error 吞成 error 流、不发 turn_ended、inflight 释放，注释标 RT-1 修后断言变）；③ **重启**：contest 形 pending_roll 无 waiter 立即掷 + 端点幂等 + 未知 eventId 仍 409；④ **注入**：透传现状**基线**（SEC1 未做——文本不限长/不剥控制字符/不识别指令，原样进 GM，注释标 SEC1 落地后断言变）。依赖 RT-2（已达成）、[RT-3](backlog-后端.md)（已达成，wave2）。注：RT-1/SEC1 真修后回炉把基线断言改正向 |
...
| O2 | feat | **core 运行时日志接入**（2026-06-25 复核·大半已做）：`cli.ts` console 面向终端输出（**保留**）；`mcp/main.ts` 等裸 error 改走 `getLogger()`。grep 复核 core 剩余裸 `console.*` 散点补走 logger 即可 | cli console 保留 / 少量裸 error | 同上 + 复核 | ✗ | ✅ 部分收尾（2026-06-26）：`packages/core/src/mcp/main.ts` 启动错误走 `getLogger().error` + `initGlobalLogger(会话目录)`（写分级文件、不污染 stdio），保留一处 stderr 兜底（init 前抛错可见）；`cli.ts` 面向终端输出按设计保留。剩余 core 裸 console 散点补点为低优先。⚠️ **实现实证/漂移**：O1 原「抽**同构** shared/logger、浏览器+node 通用」前提（前端有 console 要迁）**不成立**——core 已有 `packages/core/src/log.ts`（pino，**node-only**，分级文件），后端已接入；前端 `apps/web` **0 处裸 console** 且 web 不依赖 core。故 O-前端近乎 moot，O1 宜降级/重述为「现状已够，前端如需日志再加薄 browser logger」（建议 organize-wiki/fix-wiki-issues 修正第三批 logger 段措辞） |
