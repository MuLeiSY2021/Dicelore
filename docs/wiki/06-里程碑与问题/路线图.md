...
**已归档**（测试确认完成）
- feat：**F3 lore 侧 eval harness（offline）** → [backlog-core 主题F · F3](backlog-core.md)。✅（2026-06-26）：`src/eval/loreScenario.ts`+`loreAssertions.ts`+`eval/loreRun.ts`+`loreScenarios/frontier-saga.json`+`loreGrader.md`——确定性 mock 作者 `buildCalls`→`runBuildTool`→commit→`importPack`→`gradeLoreRun` 判运行库（零 LLM，7 测）。真构建 GM 真 LLM 一局见「待测试」。
...
- fix：**eval harness 自测覆盖** → [backlog-core · F-harness-test](backlog-core.md)。✅（2026-06-26）：`src/eval/harness.test.ts`（15 用例）覆盖 offline `runTool` 的 roll/choose/browse 等价 + 错误 + 枚举。⚠️ 漂移已记：真 harness 在 `packages/core/eval/{run,grade,batch,tool}.ts`，`choose/roll/browse` 属独立 dicelore-play MCP 包。
...
- feat：**token 用量采集**（orchestrator Agent SDK 回传处采 token、按 turn/session/agent 归因落库）→ [backlog-后端 · CO-后端-采集](backlog-后端.md)。归因维度**已自决 per-turn + per-agent 双采**（可逆）；是 [SEC2](backlog-后端.md) 计费/限流数据前置。体检实证（CROSS-TOKEN-RAW P3）：raw log 已记 usage，本项是抽取→结构化 schema→归因落库。
...
- feat：**同构 logger（O1）/ 前端日志（O-前端）** → [backlog-core O1](backlog-core.md) / [backlog-前端 O-前端](backlog-前端.md)。✅ **降级裁决：现状已满足**——core 已有 `log.ts`（pino node-only）且后端已接入、前端 `apps/web` **0 处裸 console**，「同构 shared/logger」前提不成立、O-前端 moot。
...
- feat：**e2e 玩家主线扩展（单测层）**（CROSS-E2E P1）→ [backlog-前端 · FE-e2e](backlog-前端.md)。✅（2026-06-26）：`FakeDiceGm` 教练档触发 roll/choice/game_end，单测覆盖五主线（web 64 / orch 92 测）。
