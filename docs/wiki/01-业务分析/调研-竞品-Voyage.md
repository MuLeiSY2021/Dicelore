...
- **占形态，但分主次**：anko_driver 相对 Voyage 的不同确实落在 **中文 + 多人论坛 + 安价投票 + 开源可嫁接**，但这四点**不是平级的护城河**——中文 / 多人 / 安价是**场景形态**、开源可嫁接是**分发方式**（且开源可嫁接 SillyTavern 已做到、安价机制可被快速复刻，都不构成壁垒）。真正的差异化卡位只有一条：**把"对抗讨好本能"的 GM 行为塑形框架化**——这才是 Voyage 与现有开源前端都没做的事。前三点占住 Voyage 不碰的形态，后一条才是立身之本（详见 [问题域 §2](问题域.md)）。
- **不追护城河，追先发卡位**：上述差异化里没有传统护城河——开源 + prompt/skill 塑形天然可被 fork。anko_driver 要的不是护城河，而是**先把这件事框架化、靠开源采纳占住身位、成为事实标准**。这与 Voyage 用 $43M 砌引擎护城河是两种打法：它守闭源壁垒，我们抢开源卡位（战略表态见 [问题域 §1](问题域.md)）。
