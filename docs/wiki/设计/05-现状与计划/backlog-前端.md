...
> **本页职责**：`frontend/` 层的 **issue 池**——组件 / 渲染 / 路由 / i18n / 视觉。按**主题**聚类、按 **fix/feat** 标注，广度无序（先还哪个见 [路线图](路线图.md)）。
...
> 权威设计：[玩家客户端.md](../04-子系统设计/玩家客户端.md) / [-接口.md](../04-子系统设计/玩家客户端-接口.md) / [-视觉.md](../04-子系统设计/玩家客户端-视觉.md) · [ADR-0018](../归档/历史决策日志.md)/[0019](../归档/历史决策日志.md)。
>
...
| # | 类型 | 问题 | 来源 | 恶化 | 下一步/依赖 |
|---|------|------|------|:--:|--------|
| FE-CLI-doc | docs | **README "CLI 一键开玩"断节**（USER-001，P0）：README.md L60 承诺"CLI 一键脚手架即开玩"，但 `backend/src/cli.ts` switch(cmd) 只有 new/list/inspect/init 四命令，无 play/roll/choose；真玩必须起 backend 后端 + web dev server + 浏览器，这条路径 README 没写给玩家。新玩家按 README 装完 CLI 直接卡死——"随时在线、低安装、本机直接玩到"卖点在当前实现下不成立 | 2026-06-25 全量体检 | ✗ | ✅ 达成（2026-06-26 wave4）：README 玩家入口改为 [ADR-0027](../归档/历史决策日志.md) 口径——v1 玩家入口=多端整合包（安卓/Win 优先）/自托管 docker-compose+浏览器（填 key+baseURL）；CLI 退回开发/会话管理（new/list/inspect/init，非开玩入口）；移动端=安卓/Win 优先 v1 发版优先。**仅改了英文 `README.md`**，`README.zh-CN.md` 待平行改写（见 FE-CLI-doc-zh）。 |
| FE-CLI-doc-zh | fix | **`README.zh-CN.md` 过期承诺待对齐**（随手，2026-06-26 wave4 浮现）：FE-CLI-doc 只改了英文 `README.md`，中文 `README.zh-CN.md` 很可能带同样的过期承诺（移动端长期愿景措辞 / CLI 一键开玩 / full-stack-client 措辞），需与英文 README 平行改写、对齐 [ADR-0027](../归档/历史决策日志.md) 口径 | 2026-06-26 wave4 | ✗ | 🔧 比对英文 README 改后版本，把 `README.zh-CN.md` 的玩家入口/移动端/CLI 措辞平行改写对齐 ADR-0027 |
| FE-end-lock | feat | **终局后玩家锁死无重开入口**（USER-002，P1）：`PlayPage.tsx` L365 玩家输入框 `disabled={!!gameEnd}`；L308 终局只渲染 end div，无任何"重开"按钮；grep i18n 无 restart/重来/再玩/新一局/重新开始 任何键。玩家死一次就卡死，违场景 A 卖点"随时能玩" | 2026-06-25 全量体检 | ✗ | ✅ 已裁决（[ADR-0026](../归档/历史决策日志.md) 定稿·2026-06-26 PO 复核）：v1 终局态实现口径——**删除"结束游戏"按钮**；终局后**进入复盘模式**（展示 GM 复盘 + 行动建议）；保留**"重开新局"入口**（同团本开新 session）；**不做**回滚/回溯按钮（[SNAP-1](backlog-core.md) 快照 v1 降预期、回溯续命留 v2 依赖 [ADR-0017](../归档/历史决策日志.md) 快照接线）。🔧 PlayPage 终局态渲染复盘模式（GM 复盘 + 行动建议）+ 重开新局按钮，无结束/回滚按钮；与 [backlog-core E2 ADR](backlog-core.md) 配套 |
...
| # | 类型 | 问题 | 来源 | 恶化 | 下一步/依赖 |
|---|------|------|------|:--:|--------|
| O-前端 | feat | **前端日志分级覆盖**：`frontend/src` 现 **0 日志**（无 console、无 logger），渲染/WS 重连/选项提交/错误全程无记录；按 `error/warn/info/debug` 分级覆盖 | 用户 | ✓ | 依赖 [backlog-core O1](backlog-core.md) 同构 logger 落地后接入；分级约定与 core/后端对齐 |
...
- **Tauri 壳打包**：桌面端打包。来源：用户。**2026-06-25 体检实证（PROD-005）**：从原混合条拆出独立。**关联多端整合包发版架构 [ADR-0027](../归档/历史决策日志.md)（定稿·2026-06-26 PO 复核）**——Tauri 壳是整合包发版的桌面端形态之一（Mac/Win/Linux），与安卓端并列；整合包内含本地后端 sidecar 可启停。
- **团本超开放富 UI（注入 HTML 到 play 页）** ⟶ **里程碑四**：让团本作者注入**富 UI（含任意 HTML）**到 play 页面（如崇祯模拟器式的大臣聊天/开会/战斗/国库栏），**UI 点选代替打字**；底层仍是文字冒险（不改品类 [ADR-0022](../归档/历史决策日志.md)），只是表现层超开放，让作者做出惊艳作品。**2026-06-29 用户升级期望——推翻原"廉价中间路"**（原定：框架只内置**有限几种** widget + `present:"shop"` 提示、不让团本 ship 任意 UI；商店当下走 choice 式文本菜单即可）。**安全面要紧**：团本注入 HTML = XSS 面，需净化/沙箱（关联本页已有 FE-md-xss XSS 防线）。数据层（团本如何携带/声明 UI）见 [backlog-core 未来池](backlog-core.md)。来源：用户 2026-06-22 + 2026-06-29；[声明式工具生成层 spec](../../../superpowers/specs/2026-06-22-声明式工具生成层-design.md)；主题G 关联。
- **出图图片展示（前端对话框）** ⟶ **里程碑四**：GM 出图能力的前端侧——把生成的图片在对话框展示给玩家。出图能力本体（MCP + 出图 subagent）见 [backlog-core](backlog-core.md) / [backlog-后端](backlog-后端.md) 未来池。来源：用户 2026-06-29。
...
- **移动端适配**：~~愿景 §4 长期愿景，v1 不做。~~ **2026-06-25 裁决上修（[ADR-0027](../归档/历史决策日志.md) 定稿·2026-06-26 PO 复核）**：移动端从长期愿景**提前到发版优先（安卓+Win 严重建议优先）**，走多端整合包发版架构（整合包内含本地后端 sidecar）。来源：2026-06-25 全量体检（PROD-011）+ 裁决。**需 `roadmap-delivery-workflow`（wiki 结构整理变体） 改用户与场景 §4 把"移动端长期愿景"改为"v1 发版优先"**（见 [backlog-core M1-d](backlog-core.md)）。
