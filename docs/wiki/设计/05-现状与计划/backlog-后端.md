...
| RT-FE5 | feat | **明骰内联 stream 各档 narrate 无数据源**：`RollBandSchema` 现仅 `{label,min,max}`（`packages/shared/src/presentation.ts:42`），core band spec 实带 `consequence` 文本（`harness/src/dicegm/mcp/schemas/resolver.ts:26-29`）但投影 `rollGate.ts:22-23` 丢了 → `play-roll-bands` 各档叙事无数据。A/B 未定：补 `RollBand.narration` 字段 vs 走 narration stream 末段 | acceptance-loop 0706 RT-FE5 | ✗ | ⚠️ 未裁决：A/B + narration 文本来源未定，just-in-time 补裁决（非可逆小修，schema 扩展） |
| RT-FE6 | feat | **暗骰无 WS 类型/标记**：`PendingRoll.shape` 仅 `outcome\|contest`（`presentation.ts:45`）、WS 10 类（`shared/stream.ts:16-31`）无暗骰变体；暗骰 verdict 被塌进通用 `presentation_delta`（`harness/src/dicegm/notify.ts:30-31,59-60`）无区分标记 → `play-hidden-roll`「只说判定不显结果/DC」无承载。建模承重未定：新 shape 值 vs 新 WS 类型 vs PendingRoll 加 hidden 标记 | acceptance-loop 0706 RT-FE6 | ✓（契约一旦发版难改） | ⚠️ 未裁决：承重建模决策，建议先 `brainstorming` 定型再落裁决（关联 RT-FE9 防剧透分级） |
