...
| RT-10 | feat/fix | **build turn 超时预算对大源太紧（180s 摸不完）**（2026-07-06 build-eval dogfood 发现）：GM turn 超时默认 180s（RT-1，为跑团调）；但 lore/build turn 要摸大源（实测 218KB《总之，来抽卡吧》）+ 多工具落多域，180s 内单轮推进不动实质内容——首轮只出 manifest(name+id)、次轮 `Read×13` 通读源后 180s `gm_timeout` 中止、零新产出。error 经 body 正常透传（BE-lore-error-shape）。**双因**：① 时间预算——build turn 合理耗时 > play turn，应给 lore 路径独立/更长 turn 超时（或按源大小自适应）；② 摸源效率——build-pack 教条应教 targeted `grep`+分块 `Read`（带 offset/limit）替代通读、大源分更细轮次（[report](../../../reports/2026-07-06-build-总之来抽卡吧.md) build_core_fix_hints）。 | 2026-07-06 build-eval dogfood | ✓✓（源越大越摸不完、团本造不出） | 🔧 ① lore/build turn 超时独立配置、default 拉长（如 env `DICELORE_BUILD_TIMEOUT_MS`，或 build 路径不套 dice 的 180s）；② build-pack 摸源阶段改 targeted 检索+分块读（措辞类，随手改）。①属配置/架构、②属教条措辞。 |
