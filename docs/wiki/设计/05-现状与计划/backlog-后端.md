...
| RT-7 | fix | **dice transcript 缺 `turn_end` 标记行 → rewindLast 对 dice 落空**（2026-07-02 transcript-datadir-buildeval 交付 TR3 冒泡）：`DiceGm.runTurn` 在 `yield {type:'turn_end'}` **之后**才调 `transcript.turnEnd()`，但 `streamTurn` 收到 turn_end 事件即 break → generator `.return()` 跳过 yield 之后代码 → `transcript.turnEnd()` **不可达**，dice transcript 里没有 `_:'turn_end'` 行。TR3 的 db 锚已改用 `transcript.head()`（末条 msg uuid，稳定树节点）绕开，`rewindTo(uuid)` 与 db 快照回退**正常**；但 `Rewind.rewindLast()`（回退到"上一回合末"）靠找 `_:'turn_end'` 行，对 dice 会**落空**（lore 侧经 REST 语义正常落 turn_end） | 2026-07-02 交付 TR3 | ✓（前端按回合末回退 UI 上线时才痛） | 🔧 把 `turn_end` 落 transcript 行的责任**上移到 `DiceSession`**（它已持 transcript 视图、在 turn_end 事件后仍执行），而非依赖 DiceGm 里那段 `.return()` 后不可达的代码。修完 `rewindLast` 对 dice 生效。非阻塞：核心 `rewindTo` 已可用。 |
...
| # | 类型 | 问题 | 来源 | 恶化 | 下一步/依赖 |
|---|------|------|------|:--:|--------|
| REL-datadir | feat | **数据目录布局规范未定**：现 `DICELORE_SESSIONS_DIR` 只约定了 `<dir>/{dice,lore}/sessions/<id>/` 会话数据，但**团本库(catalog)、key、配置、日志**散落无统一「数据根」约定；程序与数据也未明确分处。发版三形态（Tauri 本机 app-data / 自托管 server / dev 源码）各自「数据根」落哪、on-disk 长什么样（`config` + `catalog/` + `sessions/{dice,lore}/` + `keys/` + `logs/`）、如何整体备份/迁移/重装不丢档——均未定。牵动组合根 `server.ts` 的路径解析（core touch：sessions/catalog 路径来源） | 用户 2026-07-02（分发模型追问，选择立为正式设计） | ✓✓（团本/会话/日志随使用无限长，无统一根则备份/迁移/重装易丢档、越用越乱） | 💡 待裁：① 定「数据根」单一约定（如 `--data-dir` / OS app-data 默认）+ 其下 `config`/`catalog`/`sessions`/`keys`/`logs` 子布局；② 三发版形态各自数据根位置（Tauri app-data / server `/var/lib` 或 `--data-dir` / dev `.dicelore-eval` 同构）；③ 组合根收敛为读单一数据根、派生各子路径；④ 与 [SEC2](#主题--安全--多租户-) key 托管落点对齐。**是 eval `install.sh`/`run.sh` 的 `$` 形态之源**——eval 隔离工作区 = 该布局的一次性 dev 实例。裁决前 eval 侧先按此形态自洽落地（见 [裁决记录/transcript-runtime-and-build-eval](裁决记录/transcript-runtime-and-build-eval.md) §7，待补）。写进 [玩家客户端 决策节](../04-子系统设计/玩家客户端.md) 后关闭 |
