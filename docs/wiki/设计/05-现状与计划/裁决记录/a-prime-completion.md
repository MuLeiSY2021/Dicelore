...
> **前置：[`narrate-hook-extension`](narrate-hook-extension.md) 先行**。本裁决 §2（`front_omen` 物理子表）/§3（`anchors_json`+`surface_trigger` 列）的存储方案与 narrate-hook-extension 冲突，**已被实现事实否决**（实现走 spec 路线：`front_omen`=watcher 视图、链接=通用 `anchor` 边表、foreshadow 浮现=watcher 行，见 [内层能力库 §4.6](../../04-子系统设计/内层能力库.md)）。§2/§3 **标废弃**，待 narrate-hook-extension 批准后随本裁决整体收口时清理；§1/§4/§5/§6/§7 不受影响、仍为独立欠账。
...
## §2 front_omen 子表物化 ·【废弃·被 narrate-hook-extension 取代】

> **废弃**：实现已走 spec 路线——`front_omen` = `watcher WHERE source='front:<id>'` 视图（`frontOmenList`，已落 main），**非**物理子表。本节物理表方案否决，见 [narrate-hook-extension](narrate-hook-extension.md) §一.1 + [内层能力库 §4.6](../../04-子系统设计/内层能力库.md)。保留原文备查。
...
## §3 plotline/foreshadow 锚点 + surface_trigger ·【废弃·被 narrate-hook-extension 取代】

> **废弃**：实现已走 spec 路线——链接 = 通用 `anchor` 边表（正反查，已落 main，非 `anchors_json` 列）；foreshadow 浮现 = `source='foreshadow:<id>'` 的 watcher 行（非 `surface_trigger` 列）。本节列方案否决，见 [narrate-hook-extension](narrate-hook-extension.md) §一.2/一.1 + [内层能力库 §4.6](../../04-子系统设计/内层能力库.md)。保留原文备查。
