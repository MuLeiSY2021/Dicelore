...
- feat：**skill 加载按引用统一**（dice+lore 改用 SDK 原生 `plugins`+`skills` 按引用加载，退役 `stageSkills`/`cpSync` 暂存 + 废弃的 `allowedTools:['Skill']`）→ [backlog-后端 · H-skill-loading](backlog-后端.md)。裁决文件已备：[裁决记录/skill-loading-by-reference](裁决记录/skill-loading-by-reference.md)（**待用户勾批准**）。承重机制清理，且是 **build-agent-workspace 的前置**（cwd 交还业务的使能）；本身独立有价值（去每回合复制 + 去废弃 API + dice flow skill 借此可用 + 退役两侧内联教条兜底、教条经 skill 单路径投递+fail loud + lore 新增 `dicelore-build-core` 开场白 skill 补 dice `gm-core` 对应物）。
- feat：**构建 agent 源摄入重设计**（会话工作区 + agentic 文件构建，退役 BM25 检索）→ [backlog-后端 · 团本构建 H-build-workspace](backlog-后端.md)。裁决文件已备：[裁决记录/build-agent-workspace](裁决记录/build-agent-workspace.md)（**待用户勾批准**——挂了链接但没勾，铁律仍算未裁决）。**blockedBy [skill-loading-by-reference](裁决记录/skill-loading-by-reference.md)**（须其先行或同波）。是团本构建组件5/6 的源摄入演进，也是里程碑二 GM 质量 eval 闭环「喂真实案例建团本」的使能前置；Bash 安全欠账 [SEC4](backlog-后端.md)。
- feat：**团本构建 组件5/6 余下**（构建 MCP + Skill 侧健壮性收尾：error 上抛 / 测试覆盖 / checkout head）→ [backlog-后端 · 团本构建](backlog-后端.md)（BE-lore-error-shape / BE-lore-test-gap + BE-checkout-head）。裁决文件已备：[裁决记录/lore-build-robustness](裁决记录/lore-build-robustness.md)（**待用户勾批准**）。**组件5/6 核心已 ✅**（构建 MCP/Skill/import/toolgen），此为构建侧收尾；与 build-agent-workspace 文件重叠、同波则主 agent 集成解。
...
**未完成**（已裁决、未开发）
- feat：**token / 金钱消耗可视化**（per-turn 随 session 流走、内联每一轮，**非单独查询面板**——2026-06-30 用户纠偏）→ [backlog-前端 · CO-前端-可视化](backlog-前端.md)。采集已落库（见「已归档」CO-后端）；usage-api 聚合端点已合（留总览用）。拆 3 需求，**裁决文件待用户批准**（批准前为未裁决、不进波）：[usage-stream](裁决记录/usage-stream.md)（per-turn token 搭 turn-end 信号）→ [co-play](裁决记录/co-play.md)（跑团页每轮内联）+ [co-build](裁决记录/co-build.md)（构建页每轮内联）。
