...
**待测试**（已开发、待手动门确认）
- feat：**安装数据目录布局规范**（单一数据根 `DICELORE_DATA_DIR` + on-disk `$ROOT/{config.toml,catalog.db,keys.db,sessions/<kind>/<id>,logs}` + config.toml `[env]` + 程序↔数据分处）→ [backlog-后端 REL-datadir](backlog-后端.md)。**代码已交付**（2026-07-02 DD1–DD4，全离线测试绿），设计已沉 [玩家客户端 D5](../04-子系统设计/玩家客户端.md)；裁决 [install-datadir-layout](裁决记录/install-datadir-layout.md) 暂留。**待手动门**：真跑 `install.sh` + `run.sh -f` 起后端就绪（[eval-backend-setup](../../../../.claude/skills/eval-backend-setup/SKILL.md)）。是「可安装可用验证」前置。
