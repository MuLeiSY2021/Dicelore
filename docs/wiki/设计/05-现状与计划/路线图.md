...
- fix：**`/sessions/:id/open {ref:"head"}` → 500**（acceptance-loop 0706 RT-open-head-ref，同胞 BE-checkout-head 漏修的 open 端点）→ [backlog-后端 · RT-open-head-ref](backlog-后端.md)。**可逆 fix·方案已定（端点层解析 head）·无需裁决文件·可直接进波**。
...
**未裁决**（acceptance-loop 0706 抓出·均未到零不确定·交付前 just-in-time 补裁决，承重的先 brainstorm）
- feat：**明骰各档 narrate 无数据源**（RT-FE5·`RollBandSchema` 缺 per-band narration）→ [backlog-后端 · RT-FE5](backlog-后端.md)。A/B 未定（补字段 vs narration stream 末段）。
- feat：**暗骰无 WS 类型/标记**（RT-FE6·`PendingRoll.shape` 无 hidden + WS 10 类无暗骰）→ [backlog-后端 · RT-FE6](backlog-后端.md)。承重建模决策，建议先 brainstorm。
- feat：**loregm 缺 Draft 期校验端点**（RT-FE11·仅 `POST /catalog/validate` 整包校验）→ [backlog-后端 · RT-FE11](backlog-后端.md)。返形状文档漂移待定。
- feat：**loregm 域 WS 事件未规约**（RT-FE12·制作页 toolcalls 无事件源，同胞 G-后端-toolcall）→ [backlog-后端 · RT-FE12](backlog-后端.md)。实时通道架构决策，建议先 brainstorm。
- feat：**fake-GM 教练档 CanonScript 未接 HTTP/env**（RT-fake-gm-wiring·挡 Tier 0 curl 4 条 BLOCKED + 是 FE-e2e-browser 前置）→ [backlog-core · RT-fake-gm-wiring](backlog-core.md)。新造件设计（canon 加载 + lore 假构建），**最高优先级**。
