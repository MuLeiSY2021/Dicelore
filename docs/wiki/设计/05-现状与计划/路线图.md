...
- feat：**团本构建链路 eval + 会话对话记录(transcript)抽进 runtime + rewind + 改名**（loregm 落 `<id>_session.jsonl` 对话记录=回退入点+eval 原料；transcript 铸 UUID 权威 + append-only 树/HEAD 真回退；rewind IoC 注册器 hook 领域 rollback；db snapshot 锚 transcript_anchor；build-eval skill 用真实案例 md 驱动 loregm 建团本评构建行为+团本质量；install.sh/run.sh + 共用前置 skill；`dicelore-eval`→`play-eval`）→ [backlog-core 主题F](backlog-core.md) + [backlog-后端](backlog-后端.md)。裁决：[裁决记录/transcript-runtime-and-build-eval](裁决记录/transcript-runtime-and-build-eval.md)（零不确定设计，**待用户批准**；勾前属未裁决）。**跨里程碑依赖**：[install-datadir-layout](裁决记录/install-datadir-layout.md)（已批准）须先落（数据根 + `$ROOT/sessions/<kind>/<id>` + `DICELORE_DATA_DIR`）。
...
**未完成**（已裁决、未开发）
- feat：**安装数据目录布局规范**（程序↔数据分处 + 数据根 on-disk 形态 `config`/`catalog`/`sessions[dice|lore]`/`keys`/`logs`，三发版形态各自数据根落点）→ [backlog-后端 · 发版形态/安装数据目录布局](backlog-后端.md)。**已裁决**（[裁决记录/install-datadir-layout](裁决记录/install-datadir-layout.md)，用户 2026-07-02 批准）。「可安装可用验证」前置；分发形态已裁决（[玩家客户端 D1/D4](../04-子系统设计/玩家客户端.md)），本项补数据目录布局层。解锁 [transcript-runtime-and-build-eval](裁决记录/transcript-runtime-and-build-eval.md) §7（eval `$` 派生）。
