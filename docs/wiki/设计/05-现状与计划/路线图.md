...
**未裁决**（想法已归海拔、待设计到零不确定）
- feat：**安装数据目录布局规范**（程序↔数据分处 + 数据根 on-disk 形态 `config`/`catalog`/`sessions[dice|lore]`/`keys`/`logs`，三发版形态各自数据根落点）→ [backlog-后端 · 发版形态/安装数据目录布局](backlog-后端.md)。**「可安装可用验证」的前置**；发版**分发形态**已裁决（[玩家客户端 D1/D4](../04-子系统设计/玩家客户端.md)：双分发壳 + 多端整合包 + 自托管），本项补的是尚未落定的**数据目录布局**这层。**待设计到零不确定后再落裁决**（eval `$`=`.dicelore-eval` 形态先按此自洽，见 [裁决记录/transcript-runtime-and-build-eval](裁决记录/transcript-runtime-and-build-eval.md) §7）。
