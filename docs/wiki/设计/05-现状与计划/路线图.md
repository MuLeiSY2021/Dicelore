...
- feat：**构建 agent 源摄入重设计**（会话工作区 + agentic 文件构建，退役 BM25 检索）→ [backlog-后端 · 团本构建 H-build-workspace](backlog-后端.md)。裁决文件已备：[裁决记录/build-agent-workspace](裁决记录/build-agent-workspace.md)（**待用户勾批准**——挂了链接但没勾，铁律仍算未裁决）。是团本构建组件5/6 的源摄入演进，也是里程碑二 GM 质量 eval 闭环「喂真实案例建团本」的使能前置；Bash 安全欠账 [SEC4](backlog-后端.md)。
