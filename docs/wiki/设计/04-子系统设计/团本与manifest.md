...
> **上游依赖**：[技术选型 §5 MD主体+CSV](../03-架构/技术选型.md)、[§3 FTS5/jieba](../03-架构/技术选型.md)；[总体架构 §3 两写源/版本化分发](../03-架构/总体架构.md)、[§7 组件6](../03-架构/总体架构.md)；[02 §2 四域](../02-领域模型/核心概念.md)；[内层 §2 落库形态](内层能力库.md)、[§6 建库](内层能力库.md)；可见性 [ADR-0010](../归档/历史决策日志.md)。
...
`flows` 只声明**用哪些已存在的流程 skill**（流程 skill 本体在 [Skills 包](Skills包.md)，团本不写流程逻辑）。`clock` 缺省则退化按 `seq`（[ADR-0011](../归档/历史决策日志.md)）。
...
- **frontmatter（可选）**：`tags`（召回兜底）、`visible`（[ADR-0010](../归档/历史决策日志.md) deny-by-default，缺省隐）、`source: author`（迁移钩子，区分作者灌注 vs 运行时 AI 现编）。
...
import 时灌入 sheet 域作**开局状态**（非 world 底料——sheet 是局内可变状态）。玩家主角卡缺省可设 `visible=1`，或由 AI 开局 `sheet_show` 一次（[ADR-0010](../归档/历史决策日志.md)）。

---

## 6. `fronts/` 阵线（Front）与倒计时钟（Clock）

承接 [ADR-0016](../归档/历史决策日志.md) + [02 术语表 Front/Clock](../../术语表.md)：团本可预置"会自己推进、给世界上发条"的压力源。借自 PbtA 的 **Fronts + countdown clocks**——是作者备团的核心单元（把"团本预声明 watcher"从 [ADR-0013](../归档/历史决策日志.md) 的"留未来"提前纳入 v1）。
...
承接 [02 §2 rule 域](../02-领域模型/核心概念.md) + [ADR-0005](../归档/历史决策日志.md)：机制 / 判定规则 = `rules/*.md`，**AI 只读、人类版本化热更新**。frontmatter 带 `version`；import 落 `rule_doc`（FTS + version 字段）。曲线 / 分档这类小结构表**拍平进散文 OK**（rule 被整段读、不抽样、不过滤，[内层 §2](内层能力库.md)）。
