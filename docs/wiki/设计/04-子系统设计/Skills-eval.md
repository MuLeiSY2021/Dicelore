...
- **play-eval skill**([`.claude/skills/play-eval/SKILL.md`](../../../../.claude/skills/play-eval/SKILL.md))：教 CC 经 play-mcp 跑 eval + 写报告的流程 skill(起后端 → 配 play-mcp → 跑局 → 按教条口径评估 → 写报告到 `reports/`)。⚠️ **该 skill 仍是旧 baseline 双档流程，待按真实案例对照重新设计**（评测对照系修订；且其代码路径引用受 eval 框架重构影响、需一并更新）。
