...
> **上游依赖**：[02 §4 塑形层 / 三失败×三杠杆矩阵](../02-领域模型/核心概念.md)；[总体架构 §5 塑形层③ / §6 一轮+三流](../03-架构/总体架构.md)；[技术选型 §2 MCP+Skill](../03-架构/技术选型.md)；[跨agent §2/§4 装载走 Claude Code skill 机制](../03-架构/跨agent与适配层.md)；Principles 焊进（见本页「决策与权衡」节）。平级：[MCP 工具面](MCP工具面.md)（skill 要教 AI 用的工具 + 补刀挂载点）、[内层能力库](内层能力库.md)（仅参考，Skills 不碰存储）。
