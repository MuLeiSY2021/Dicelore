...
> **上游依赖**：[总体架构 §2 两层工具 / §3 数据工具 / §3.1 可见性 / §4 裁决 / §4.1 narrate / §6 一轮+三流](../03-架构/总体架构.md)；[技术选型 §1 Zod / §4 stdio](../03-架构/技术选型.md)；[02 §2 四业务域+要点5 / §3 行动层](../02-领域模型/核心概念.md)；[ADR-0007 状态骰下沉 / 0009 narrate 升格 / 0010 可见性](../归档/历史决策日志.md)；求值与存储语义 → [内层能力库](内层能力库.md)。
...
- **expr 全程是字符串、MCP 不解析**（[内层 §3.1](内层能力库.md)、[ADR](../归档/历史决策日志.md)）：凡 `expr` 字段都是 `"1d20 + {张三.力量}"` 这样的串，MCP 原样透传给内层求值器，本层不拆。
- **AI 只给引用、不给真实数值**（铁律，[03 §4](../03-架构/总体架构.md)）：靠 expr 的 `{实体.属性}` 约定 + 内层引擎取真值；**schema 不强卡**（自由串拦不住硬编数字），降级为 L2 教 + L3 账本审计（与状态骰下沉同构，[ADR-0007](../归档/历史决策日志.md)）。
...
- **暂存语义**（[ADR-0009](../归档/历史决策日志.md)）：本调用只把"下轮选项+后果"暂存进会话态，**轮内可反复调用改写、末次为准**；AI 的回合自然结束时，**Stop hook 物化**——落一条选项+后果 event（seq 在玩家 pick 之前 → L3 可验"后果在先"）、由输出层呈现给玩家。
...
- **三档结果（PbtA 对齐，[ADR-0016](../归档/历史决策日志.md)）**：`bands` 是团本/AI 定义"三档结果"的落点——典型应是**完全成功 / 部分成功（成功但有代价）/ 失败（有后果）**而非二元成败，"零代价完全得手"是窄档、非默认。每档 `consequence` 即该档的 fail-forward 后果（[Skills 包 §3](Skills包.md)）。
...
// out —— 账本，每项标 rolled|set，喂 L3
{
  ...
  fired_watchers?: [{ watcher_id, payload, condition }],  // 本次写就地触发的 watcher（[ADR-0013](../归档/历史决策日志.md)）→ AI 当轮即时反应
  event_id,                                   // 落一条 kind=mutation event（每个触发的 watcher 另落一条 kind=watcher_fired）
}
```

- **状态骰已下沉**（[ADR-0007](../归档/历史决策日志.md)）：带骰项（`HP-2d6`）引擎内掷、AI 给不出真值；纯赋值/集合增减同批，**整批原子**，非数值算术报错+整批回滚。
...
watcher_set: {                                // 命名 dicelore_watcher_set（独立名；泛化并取代旧 timer_set，[ADR-0013](../归档/历史决策日志.md)）
...
//   是「未结张力看板」（[问题总账 主题A](../05-现状与计划/问题总账.md)）的第一块拼图——仅覆盖 watcher 一处，note/暗格的聚合视图仍待设计周期。
```
...
承接 [03 §3.1](../03-架构/总体架构.md)、[ADR-0010](../归档/历史决策日志.md)。GM 用这三个工具控制"玩家能看到什么"；输出层据 `visible` 过滤渲染。
...
承接 [03 §4.1](../03-架构/总体架构.md)、[ADR-0009](../归档/历史决策日志.md)：散文 stream、轮内可多次、**非终结步骤**。
...
- **无 `game_time`**（time 不升格，[ADR-0011](../归档/历史决策日志.md)）；不带"本回合裁决引用"（L3 靠"一个 agent 回合"窗口机械圈定，[内层 §4.2](内层能力库.md)）。
...
> **回滚 / 快照不在本工具面**（[ADR-0017](../归档/历史决策日志.md)、[内层 §4.5](内层能力库.md)、[adapter §8](adapter与L3审计.md)）：撤回 / branch / swipe 是**玩家元动作、非游戏动作**——快照由 Stop hook 写、回滚由 UserPromptSubmit hook auto-sync（兜底 CLI），**不给 AI 工具**（防 AI 把回滚当叙事手段滥用）。故上表无 `rollback` / `checkpoint` 类工具。
...
§1–§7.1 的工具是**框架硬编码**的运行时工具面（`mcp/tools.ts` 的 `TOOLS`：resolver/sheet/event/world/io 五族；运行时 mcp 工具面在 `harness/src/dicegm/mcp/**`，toolgen 编译引擎纯件在 `backend/src/toolgen/**`，[ADR-0028](../归档/历史决策日志.md)）。spec §9 定「生成工具并入同一工具面」——团本 `tools:` 声明 + 框架标准库声明经 `toolgen.compileTool` 编译后，**与硬编码工具并列注册进同一 `createMcpServer`**，GM 侧无感（同 `dicelore_` 前缀，类别走 MCP annotation `source:"generated"`，[spec §6](../../../superpowers/specs/2026-06-22-声明式工具生成层-design.md)）。
...
> **本页主体（§1–§7）是运行时工具面**（GM 跑团：resolver / sheet / world / narrate……）。**构建期工具面另属一套**——作者侧、构建期专属，挂在 `LoreSession` 的 in-process 构建 MCP（[团本构建工具链 §6](团本构建工具链.md)、[后端双路径架构 §5](后端双路径架构.md)），与运行时 `TOOLS` **编译期不交叉**（[ADR-0023](../归档/历史决策日志.md) ⑥）。两套工具面互不出现在对方场景。本节记其权威 schema。
...
### 8.2 front md 正典 + prologue 必备（本轮新增 2，[ADR-0024](../归档/历史决策日志.md)）
...
> 本节折叠落在「MCP 工具面」这一层（工具 schema / 分流 / 工厂接缝）的关键设计决策与被否方案——**精炼重述，全文见 [归档/历史决策日志](../归档/历史决策日志.md)**。同一条决策横切多页时，本节只讲与本层（工具面形状）相关的面。

<details>
<summary>narrate 升格散文 stream + 一轮范式 + 输出层三流（ADR-0009）</summary>
...
<details>
<summary>明暗骰工具分流：<code>_hidden</code> / <code>_open</code> 拆名、不加布尔参（ADR-0019）</summary>
...
<details>
<summary>createMcpServer 工厂 + onCanonWrite 写后接缝（ADR-0020）</summary>
