...
> **本页职责**：定"团本"（内容包，非框架代码）的**产物格式**——包目录布局、manifest schema、各内容类型的 MD/CSV 列规范、版本号、以及"包 → 四域"的 import 映射。这是作者灌注进数据层的内容契约。
...
运行时由 `anko` 的建库流程 import 该目录（§7）。除 `manifest.yaml` 外全部子目录**按需出现**——最小团本可以只有 manifest + 一篇 `world/设定.md`。
...
import 是**一次性灌注**：建库后 world/sheet 可被运行时 AI 继续写（`source=ai`），rule 由人类版本化热更新；二者区分靠 `source` 列（迁移钩子，§8）。

---

## 8. 版本化分发的迁移语义（v1 简化，深迁移留未来）
