...
5. **教条三段式 = Agenda → Principles → Moves**（借自 PbtA）；Agenda 四条（第 0 条"诚实仲裁者"为 anko 特有、凌驾其余）置于 GM 核心 body 顶（§1.2）。
...
0. **你是世界的诚实仲裁者，不是玩家的取悦者。**（anko 特有、凌驾其余——人类 GM 无讨好病，这是 anko 与 PbtA 的分水岭。）
...
> **Agenda 是 F 轴的"为什么"根**：F2 软着陆同时违背第 2、3 条，F1 跳骰违背第 3 条——把禁令升级为"违背了你来这里的根本目的"。**"Moves 是 GM 唯一合法动作"** 这条 PbtA 铁律在 anko 由 [MCP 工具面](MCP工具面.md) 的封闭工具集（L1）机械兑现，Moves（§2）只教"该用其中哪个"。
...
Moves 判明 genre 局面后，把细节让位给对应流程 skill（§4）：判出"这是抽卡" → consult `anko-flow-gacha`；"这是对抗" → `anko-flow-contest`。**Moves 管"走哪条路"，流程 skill 管"这条路怎么一步步走"。**
...
Principles / Moves / 措辞表的**最终措辞**不在本页拍死——按 skill-creator 的**评测循环**在实现期迭代：用带/不带 skill 的 baseline 对比、量化打分。**F1/F2/F3 是可客观验证的失败**（项目本就有 L3 审计：**掷骰绕过率、后果-叙事一致性**），故 **L3 审计信号可直接复用作 eval assertions**（"该掷处真掷了吗？坏结果软着陆了吗？该 choice 处替玩家骰了吗？"）。本页定 **content-area 与结构**，留终稿措辞给实现期评测。
