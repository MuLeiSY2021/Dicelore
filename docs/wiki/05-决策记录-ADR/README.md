...
- **v1 降预期补注（2026-06-25 全量体检裁决，追加式不回改正文）**：本 ADR 原定 branch 进 v1、swipe 默认重掷等手动回滚交互**降级为 v2**。**v1 只做自动持久化（存档/读档）**——① snapshot 表建表 + core `checkpoint()`/`restore()` 原语 + `SnapshotParticipant` 注册表；② orchestrator `DiceSession.turnEnd` 调 `checkpoint()` 自动写快照；③ REST `POST /sessions/:id/rewind` 自动恢复最近快照；④ 前端 PlayPage 加存档/读档入口（自动恢复）。**v2 留**：手动回滚按钮 + branch/swipe + 终局续命（[ADR-0026](#) game_end 后回溯续命）+ 死错反悔 + 超时 restore 接线（[backlog-后端 RT-1](../06-里程碑与问题/backlog-后端.md) 长期方案）。**裁决理由**：v1 开放手动回滚需补 snapshot 表 + core 原语 + Stop/UserPromptSubmit hook 接线 + REST 端点 + UI 五层，scope 过大且与终局机制（[ADR-0026](#)）耦合；v1 先把自动持久化做掉守"随时能玩"卖点，回滚/branch 等交互留 v2。落 [backlog-core SNAP-1](../06-里程碑与问题/backlog-core.md)。
...
- **决策**：
  - **① eval 经 play-mcp 连真后端（缝B）**：弃"裸 CC harness"路线。play-mcp 包后端 play HTTP 为 8 工具（list_scenarios/open_session/start_game/send_message/get_presentation/choose/roll/browse）；narration 只经 WS 流式（`streamDriverTurn` 不落库），故 `send_message`/`start_game` 内部连 WS 收 `narration_commit`→`turn_ended` 返回 GM 散文，`get_presentation` 取机械态快照。玩家视图 = narrations(WS) + presentation(HTTP)。CC 当玩家+评估者。~~两档对照 doctrine（带 gm-core）vs baseline（`DICELORE_BASELINE=1` 去教条）。~~ → **（2026-06-29 修订）对照系改为 `docs/research` 真实案例（兽人/抽卡/恶龙团→构建团本→跑团→对照真实案例的定性评判），废 doctrine-vs-baseline A/B、量化不可行→定性报告（见下方「eval 对照系修订补注」）。**
  ...
  - **③ ~~baseline 接线~~ → （2026-06-29 废除作为证据系，见修订补注；`DICELORE_BASELINE` 接线代码留作可选消融实验、不再是教条价值判据）**：~~`DiceSessionDeps.baseline?` → `buildBaselinePrompt`（去 doctrine）+ 强制 skills 空；`server.ts` 读 env `DICELORE_BASELINE=1` 透传。分离「教条有无」作对照变量。~~
- **后果**：落 [Skills-eval §0/§4/§5](../04-子系统设计/Skills-eval.md)（路线改 play-mcp）、`apps/orchestrator/src/server.ts` + `packages/core/src/index.ts`（路径对齐）、`apps/orchestrator/eval/play-mcp.ts`（eval 入口）、[.claude/skills/dicelore-eval](../../../.claude/skills/dicelore-eval/SKILL.md)（eval 流程 skill）。**2026-06-24 RUN_LIVE 通路验证通过**：orc-hunt doctrine 档，真 GM（glm-5.2）+ 种子生效 + WS 事件流闭环（见 [reports/](../../../reports/)）。**首份 finding**：`stripReasoning` 只剥 `<think>` 标签块，glm-5.2 无标签英文思考段泄漏成 narration（narrate-leak）——靠 gm-core 教条约束（别吐思考），记 [findings B7](../../../packages/core/eval/findings.md)。**未结**：F2 终局观测待多轮跑；完整语料对标待 dicelore-eval skill 跑。**被否**：① in-process harness（绕过后端 HTTP/WS、测不到缝B）；② 后端保留平铺路径 + play-mcp 适配（违"以 core 为准"、留两套路径规则）。
- **eval 对照系修订补注（2026-06-29，PO 复核已认可，追加式不回改正文）**：原决策①「CC 当玩家+评估者，**两档对照 doctrine vs baseline**（`DICELORE_BASELINE=1` 去教条）」+ 决策③「baseline 接线」作为「**教条价值证据系**」的地位**废除**。
  - **改为**：eval 的对照系 = **`docs/research/` 真实安价案例**（兽人冒险 / 抽卡 / 恶龙团，nmbxd 实跑串）。**闭环**：把真实案例喂进**团本构建库** → 和 lore GM 对话建出团本 → **人工认可团本质量** → 让 **Claude 的 skill + MCP** 拿这个团本跑 eval → **落盘 eval 报告**，对照真实案例评判 GM 表现。
  - **量化**：以目前认知**不可行** → 闭环产出**定性报告**（量化口径待探索），不强求"with 应显著低于 baseline"那类数值结论。
  - **理由**：doctrine-vs-baseline A/B（有无教条）回答的是「教条本身贡献几何」的消融问题；但真正要回答的是「我们的 AI GM 跑得像不像真实安价好作品」——这个**只能以真实人类跑出来的案例为对照系**。baseline A/B 答非所问，且诱导"量化教条价值"的伪精确。
  - **被否**：保留 baseline A/B 作主证据系（答非所问）；`DICELORE_BASELINE` 接线代码可留作**可选消融实验**，但**不再是教条价值的判据**。
  - **后果（传播，待本补注定稿后做）**：[wiki 01 成功标准](../01-业务分析/成功标准.md) / [04 Skills-eval](../04-子系统设计/Skills-eval.md)·[Skills包](../04-子系统设计/Skills包.md)·[团本构建工具链](../04-子系统设计/团本构建工具链.md) / `packages/core/eval/grader.md` / [dicelore-eval skill](../../../.claude/skills/dicelore-eval/SKILL.md) 的 baseline 框架移除或重构为「真实案例对照」；[路线图里程碑二](../06-里程碑与问题/路线图.md) + [backlog-core 主题F](../06-里程碑与问题/backlog-core.md) 已先行改对（决策已落 06）。

---

## ADR-0026 终局机制：GM 自调 game_end + 软判据 + 玩家主动结束 + 终局后进复盘模式（定稿·2026-06-26 PO 复核）

> **状态：定稿（2026-06-26 PO 复核）**（2026-06-25 全量体检未决问题 2 主流程裁决落账，2026-06-26 PO 复核定稿）。对应 [backlog-core E1/E2](../06-里程碑与问题/backlog-core.md) + [backlog-前端 FE-end-lock](../06-里程碑与问题/backlog-前端.md)。
>
> **2026-06-26 PO 复核修改**：大方向认可，决策④一处修改——**删除"结束游戏"按钮，终局后直接进入"复盘模式"**（GM 给复盘 + 行动建议，原"要 AI 建议"升为默认去向）；**回溯续命留 v2**。其余三连（① GM 自调 game_end、② 软判据、③ 玩家主动结束权）原样维持。修改点见决策④及末尾补注，原推导保留。
...
- **决策**（四连）：
  ...
  - **④ game_end 后直接进入「复盘模式」（2026-06-26 PO 复核修改）**：v1 **删除"结束游戏"按钮**，终局后**直接进入复盘模式**——GM 给"复盘 + 如果继续你可以..."的行动建议（原候选"要 AI 行动建议"由可选去向升为**默认去向**），从复盘模式可**重开新局**。**回溯续命**（状态回退到死亡前叙事继续，**非对话撤回**，依赖 [ADR-0017](#) 快照）**留 v2**。v1 终局态＝复盘模式（GM 复盘 + 行动建议）+ 重开新局入口，**无结束按钮、无回滚/回溯按钮**（见 [FE-end-lock](../06-里程碑与问题/backlog-前端.md)）。
  - **～～④ game_end 后玩家三去向（原草案，2026-06-26 PO 复核改为「进复盘模式」，见上）～～**：~~①**结束游戏**（退出/回主页）②**要 AI 行动建议**（GM 给"如果继续你可以..."的建议，非续局）③**回溯续命**（状态回退到死亡前叙事继续，**非对话撤回**，依赖 [ADR-0017](#) 快照，**未来/v2**——v1 只给"结束/重开新局/要 AI 建议"三按钮，回溯续命 v2 做见 [FE-end-lock](../06-里程碑与问题/backlog-前端.md)）。~~
- **后果**：落 [backlog-core E1/E2](../06-里程碑与问题/backlog-core.md)（去待裁决、写裁决结论）、[backlog-前端 FE-end-lock](../06-里程碑与问题/backlog-前端.md)（v1 删结束按钮、终局后进复盘模式、无回滚/回溯）、[路线图第四批](../06-里程碑与问题/路线图.md)。E1 教条措辞待 F2 harness 多轮跑测校准（"GM 自收判据"是行为类、需 mock 玩家↔真 Claude-GM 验证）。**与 [ADR-0017](#) 快照的关系**：回溯续命 v2 依赖快照接线（v1 快照降预期只做自动持久化、手动回滚 v2，故回溯续命也 v2）。**被否**：① 框架据 sheet 钟/watcher 自动敲 game_end（HP<=0 等硬条件误判复活等可逆状态，E2 候选 watcher 谓词的核心矛盾未解）；② 纯 GM 自调无玩家主动结束权（GM 讨好本能下永不收局无兜底）；③ game_end 后只能结束游戏无重开/复盘（玩家死一次卡死，违场景 A "随时能玩"卖点——故终局后默认进复盘模式而非结束）。
- **2026-06-26 PO 复核修改补注**：大方向认可，**一处修改**——决策④从"v1 三按钮（结束/重开新局/要 AI 建议）+ 回溯续命 v2"改为「**删除"结束游戏"按钮、终局后直接进入复盘模式**（GM 复盘 + 给行动建议，原"要 AI 建议"升为默认去向）、从复盘模式可重开新局；**回溯续命留 v2**」。其余三连（①②③）原样维持。被否项不变（被否③"终局后只能结束游戏"那一极端与新④"进复盘模式"不冲突，仍是被否的反面）。

---

## ADR-0027 多端整合包发版架构：server docker-compose 自托管 + 客户端整合包 + 安卓/Win 优先（定稿·2026-06-26 PO 复核）

> **状态：定稿（2026-06-26 PO 复核，原样通过）**（2026-06-25 全量体检未决问题 4 主流程裁决落账，2026-06-26 PO 复核原样定稿、内容不改）。对应 [backlog-前端 FE-CLI-doc](../06-里程碑与问题/backlog-前端.md)。
...
- **后果**：落 [backlog-前端 FE-CLI-doc](../06-里程碑与问题/backlog-前端.md)（去待裁决、写多端发版口径）、[backlog-前端 Tauri 壳/移动端未来池](../06-里程碑与问题/backlog-前端.md)（加注关联本 ADR）、[backlog-core M1-d](../06-里程碑与问题/backlog-core.md)（加注移动端提前、需 `roadmap-delivery-workflow`（wiki 结构整理变体） 改用户与场景§4）、[路线图第三批](../06-里程碑与问题/路线图.md)。**与 [SEC2](../06-里程碑与问题/backlog-后端.md) API key 托管配套**：整合包内含本地后端 sidecar 托管 key（前端 localStorage 不存 key 只存引用）。**与 [ADR-0018](#) 的关系**：本 ADR 是 ADR-0018"双分发壳（Tauri 个人向 + Web 企业向）"的发版形态细化——整合包 = Tauri 壳 + 本地后端 sidecar 打包；自托管 = Web 壳 + 远程后端 docker-compose。**被否**：① 补 CLI play 子命令（要么连后端没解决低安装、要么内嵌后端 CLI 变重）；② 只给 web 客户端不给整合包（目标用户不惯命令行/起后端、违"开箱即用"）；③ 移动端留长期愿景（目标用户主力设备是手机，v1 不做移动端=放弃主力场景）。

---

## ADR-0028 storage-port 四根重构：apps/orchestrator+core 溶解为 frontend/backend/harness/packages + 依赖倒置存储端口（定稿·2026-06-29）

> **状态：定稿（2026-06-29）**。结构权威页——本 ADR 是新「四根」目录结构 + storage-port 依赖倒置的单源；下游结构表述（[03 总体架构](../03-架构/总体架构.md) 等）引本 ADR，不另起一套解释。**追加式更新**前序 ADR 的结构落点（见末段），不回改 ADR-0001~0027 正文（含其中 `apps/orchestrator`/`packages/core` 字样——历史记录）。

- **背景**：旧布局 = `apps/{web,orchestrator}` 两 app + `packages/{core,shared}`，其中 **`core` 是 grab-bag**（store/resolve/present/catalog/build/toolgen/expr/eval/mcp/adapter/dice/skills 全堆一处）。把 core 拆成职责分明的根时撞到**包级循环依赖**：`backend/api → harness/DiceSession → harness/mcp → backend/store` 成环——`orchestrator` 既驱动会话（`api → DiceSession`）、会话的 mcp 工具面又**直读 store**（`mcp → store`），于是 `@dicelore/backend` 与 `@dicelore/harness` 互相 import。**单纯挪文件解不掉，必须断一条边**。用户决策（2026-06-26）：不靠挪位避环，而是建 storage-port——因为目标本就是「agent 不直连 store、经接口声明存储意图、后端将来可加 cache / 远程 / 多租户」，与 anti-F1 铁律（[ADR-0019](#adr-0019-玩家闸控明骰明暗骰名分流--阻塞式-resolver--anti-f1-边界)：AI 只给引用、MCP 执行）同构，推进一层：**mcp 只声明存储意图、backend 执行**。
- **决策**（四连）：
  - **① 溶解 core，重组为四顶层根**：`frontend/`（玩家/作者客户端 UI，React+Vite，`@dicelore/frontend`）、`backend/`（存储 + 服务宿主：store 四域/FTS、resolve 裁决、present 呈现、catalog 团本库、build 构建纯逻辑、expr 取数文法、toolgen 声明式工具编译、eval 纯件、session KV、`sessionBackend.ts`（端口实现）、api（HTTP/WS 薄层）、server.ts、cli.ts，`@dicelore/backend`）、`harness/`（agent 运行时：`runtime/` 共享骨架 Session/registry/streamTurn/wsHub/agent/reasoning、`dicegm/` dice 跑团线、`loregm/` lore 构建线、`eval-dicegm/`·`eval-loregm/` 评测脚本，`@dicelore/harness`）、`packages/`（纯库：`interface`/`dice`/`errors`/`logs`/`shared`）。外加 `adventures/`（预制团本内容包，非代码根）。
  - **② 依赖倒置建 storage-port（断模块级环的关键）**：中立包 `packages/interface` 定义 backend 面向 harness 的**端口接口**，按领域分组——`Store`（sheet/event(log)/world/rule/watcher/pendingChoice/pendingRoll/mutations）、`Resolver`（resolveContest/resolveOutcome/commitPendingRoll）、`Snapshots`（checkpoint/restore/latest/list）、`Catalog`（importPack 开局物化 + 读）、`Presentation`（buildPresentationModel）、`Meta`（metaGet/metaSet）、`Toolgen`（toToolDef/ToolDecl）——聚合为一个会话级实例 `SessionBackend = Store & Resolver & Snapshots & Catalog & Presentation & Meta & Toolgen`。**harness 只依赖端口**（`createMcpServer`/`DiceSession` 收 `SessionBackend` 而非 `db + 直接 import`）；**backend 实现端口**（`openSessionBackend(db)`：把 store/resolve/present/... 函数用闭包绑定 db、方法不暴露原始 db）；**组合根 `backend/server.ts` 按会话注入 harness**（开局：解析会话 db → `importPack` 物化 → `openSessionBackend(db)` → 传 harness 会话工厂建 `DiceSession`）。断环原理：`harness→backend` 这条边被反转为 `harness→interface←backend`，只剩 `backend→harness`（组合根驱动会话）单向边，**模块级无环**。`openSession` 路径策略（appDataRoot 等）不进端口——是宿主决策，归 `harness/runtime/paths`。
  - **③ 命名约定：端口接口不带 `Port`/`I` 后缀**（C#/Java 遗留装饰；TS 接口与实现可同名共存，backend 直接 `class Store implements Store`）。「端口」只作架构叙述词、不进类型名。
  - **④ 各模块按成熟惯例内部分层**（barrel/exports 不变 → 外部零改动、影响锁包内）：**frontend** = feature 切片（`app`/`shared`/`features/{play,catalog,build,home,config}`/`shell`/`styles`）+ `@/*` 路径别名（消除脆弱相对路径），不引 feature 桶文件（护 tree-shaking）；**backend** = `store/` 按业务域分组（`sheet/event/world/narrative/interaction`，`db/views/fts`+`snapshot/usage` 留顶层基础设施），api/expr/resolve/present 等已对齐惯例不动；**harness** = `mcp/` 与 `adapter/` 下沉 `dicegm/`（实证 loregm 零引用、本是 dice 私有），skills 按角色归位（`gm-core`/`flow-*` → `dicegm/skills/`、`dicelore-build-pack` → `loregm/skills/`），`runtime/` 真共享不动。
- **后果**：四根成立、**模块级无环**；backend 可换存储实现 / 加 cache / 远程化而 harness 完全无感（端口隔离存储细节）。eval 迁 `harness/eval-dicegm`（play-mcp/run-live）+ `harness/eval-loregm`（build-mcp），**删 `apps/`**。**包级 `harness↔backend` 互指——裁决：接受**（2026-06-29，重构遗料清理 #7 核心时复核定案）：剩余 4 处 import 全是**组合根/入口**——`harness/dicegm/mcp/main.ts`（stdio MCP 入口，非死料：`adapter/init` 写的 `.mcp.json` 指向它）+ 3 个 CC hook 脚本 `adapter/hooks/{session-start,turn-start,turn-end}.ts`；composition root 在入口把抽象接到具体实现是其本分（Fowler）、不算耦合违例，且 **hooks 是 CC spawn 的独立进程脚本**（从 env 自举 `openSession`+`openSessionBackend`，结构上无「收注入」接缝、改注入要改 CC hook 执行模型）。包级互指是 **cosmetic**（npm workspaces 软链双向、TS 无 project references、运行时无顶层求值环——gate 全绿为证），断它唯一办法是把 4 入口整体搬去 backend 侧（动 hooksDir/settings.json 生成/cli init 接线，碰运行期 hook 安装资产），**代价 > 收益**故不断。单源注记落 `harness/src/index.ts` barrel 顶。**实测教训**（2026-06-26 首次跳步翻车）：先注册 workspace 再搬码（否则 install 删手动软链级联假错）；跨层共享类型（如 `ToolDef`）先下沉中立处再搬模块；barrel 用显式 re-export 别自动 `export *`；严格按「interface 类型（无悔）→ 小步搬、每步绿」顺序，不造破环中间态。
- **本 ADR 追加式更新前序 ADR 的结构落点**（不回改其正文）：**[ADR-0023](#adr-0023-后端双路径架构lore-构建--dice-跑团一后端共享底层) ⑩「orchestrator 三分布局 `api/pkg/dice·lore`」** → 物理落点迁为四根（api/store/引擎 → `backend/`，pkg 共享骨架 → `harness/runtime/`，dice → `harness/dicegm/`，lore → `harness/loregm/`）；**[ADR-0020](#adr-0020-组件7-实时引擎面dicelore-mcp-in-process-挂载--oncanonwrite-写后接缝--gmdriver-抽象) 缝A「dicelore MCP in-process 同进程挂载」** → 仍同进程，但物理上经 storage-port 注入（`backend/server.ts` 组合根注 `SessionBackend` 给 harness 会话）；**[ADR-0025](#adr-0025-eval-经-play-mcp-连真后端缝b--后端-session-路径对齐-core) eval 入口 `apps/orchestrator/eval/play-mcp.ts`** → 迁 `harness/eval-dicegm/`。这些是物理落点迁移、设计推理（双路径/缝A同进程/eval 经缝B）不变。
- **被否**：① **挪位避环**（把 api 放进 harness）——能断环且零端口，但拿不到「后端可换存储 / 加 cache / 远程化」目标收益（用户明确要端口本身的价值）；② **store 留 `packages/`**（backend 与 harness 同依赖 packages/store，无环最省事）——但 mcp 仍直连 store、要拆的耦合还在，仅作「先不做端口」退路存在，本 ADR 选了做端口故不取；③ **断包级 harness↔backend 环**——唯一断法搬 4 入口去 backend，为 cosmetic 收益冒运行期 hook 安装风险，代价 > 收益故接受不断。
