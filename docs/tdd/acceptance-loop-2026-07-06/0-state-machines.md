...
stateDiagram-v2
  ...
  落地 --> 配置页 : 去配置
  落地 --> 跑团页 : 继续最近会话（若有）
  note right of 落地 : 核心 = **指南**（怎么用 + 使用手册在哪）；**只显示最近一个会话摘要**，不投影全量列表（全量会话在跑团/制作页顶栏 bar）
```
...
### B4. 跑团页（第 3 页 · 顶栏 session bar + 投影 dicegm 域机 A2）

```mermaid
stateDiagram-v2
  [*] --> 无活动会话
  无活动会话 --> 未开场层 : 从团本目录「开始游戏」进入 / 点顶栏某会话
  ...
  续玩层 --> 终局画面 : game_end
  终局画面 --> 续玩层 : 继续（续玩 / 新局）
  note right of 续玩层 : **顶栏 session bar**：滚动刷新列**所有会话**（最近活动日期 / 所属团本 / 最新回复内容），点击切换。投影 dicegm 域机(A2) + 面板三态(dock/弹出/隐藏) + 活动轨源浏览(设定/工具) + 断线重连(快照+events 回填)
```
...
stateDiagram-v2
  ...
  选内容类型 --> 查看编辑 : 选中（世界设定/NPC/卡池/规则·分档/Front/Manifest）
  查看编辑 --> 选内容类型 : 切类型
  ...
  note right of 查看编辑 : **顶栏 session bar（同跑团页）**：滚动列所有**构建会话**（活动日期/团本/最新）。投影 loregm 域机(A3)。右栏=构建助手对话(显示调了哪些工具)+整包校验报告(error/warn 计数+定位)；上下文条=团本名+草稿版本
```
