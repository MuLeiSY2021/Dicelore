...
- docs/wiki/05-决策记录-ADR/README.md → ADR-0018（立项五连：GUI 提前 / Agent SDK headless host / 一契约两壳 / 玩家选择捕获 / 自定义 MCP 周边边界 + webhook 通知缝 + 几乎不改 `@dicelore/core`）
...
我要做的是【从下面三选一，删掉不要的】：
A) 转 writing-plans，把上面 spec 出成实现计划。接 dicelore MCP 那步对接组件2（MCP 工具面，已合并于 `@dicelore/core`）；`packages/shared` 契约类型 / orchestrator 的 presentation.ts / web 骨架可并行先做。
...
**仓库**：同仓 workspace 多包（薄 `private` root + 引擎在 `packages/core` = `@dicelore/core`，加 `apps/*` + `packages/*`）。
**与 core**：单向依赖、几乎不改 `@dicelore/core`；**唯一例外** = 组件2 MCP server 加可选 notify 模块（webhook 通知缝）。
...
**硬阻塞**：组件2 已合并（`@dicelore/core` 的 `TOOLS`/`runTool`/notify 出参就绪），orchestrator 接 MCP 不再阻塞；`packages/shared` / `presentation.ts` / web 骨架可并行先做。
