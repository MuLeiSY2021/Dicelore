...
> **上游依赖**：[总体架构 §2 两层工具 / §3 数据工具 / §3.1 可见性 / §4 裁决 / §4.1 narrate / §6 一轮+三流](../03-架构/总体架构.md)；[技术选型 §1 Zod / §4 stdio](../03-架构/技术选型.md)；[02 §2 四业务域+要点5 / §3 行动层](../02-领域模型/核心概念.md)；状态骰下沉 / narrate 升格 / 可见性等决策见本页「决策与权衡」节；求值与存储语义 → [内层能力库](内层能力库.md)。
...
- **expr 全程是字符串、MCP 不解析**（[内层 §3.1](内层能力库.md)）：凡 `expr` 字段都是 `"1d20 + {张三.力量}"` 这样的串，MCP 原样透传给内层求值器，本层不拆。
...
承接 [02 §3](../02-领域模型/核心概念.md)、[03 §4](../03-架构/总体架构.md)。独立裁决工具——共性：**随机/取真值全在引擎内执行，AI 给不出也改不了真实结果**（anti-F1/F2），且**各落一条"裁决记录"event 供 L3 比对叙述**。
...
//   是「未结张力看板」（[backlog-core 主题A](../05-现状与计划/backlog-core.md)）的第一块拼图——仅覆盖 watcher 一处，note/暗格的聚合视图仍待设计周期。
```
...
- **无 `game_time`**（time 不升格，见内层能力库决策节）；不带"本回合裁决引用"（L3 靠"一个 agent 回合"窗口机械圈定，[内层 §4.2](内层能力库.md)）。
...
> **回滚 / 快照不在本工具面**（快照决策见 [内层 §4.5](内层能力库.md)、[adapter §8](adapter与L3审计.md)）：撤回 / branch / swipe 是**玩家元动作、非游戏动作**——快照由 Stop hook 写、回滚由 UserPromptSubmit hook auto-sync（兜底 CLI），**不给 AI 工具**（防 AI 把回滚当叙事手段滥用）。故上表无 `rollback` / `checkpoint` 类工具。
...
> **本页主体（§1–§7）是运行时工具面**（GM 跑团：resolver / sheet / world / narrate……）。**构建期工具面另属一套**——作者侧、构建期专属，挂在 `LoreSession` 的 in-process 构建 MCP（[团本构建工具链 §6](团本构建工具链.md)、[后端双路径架构 §5](后端双路径架构.md)），与运行时 `TOOLS` **编译期不交叉**（构建期工具面隔离，见 [团本构建工具链](团本构建工具链.md) / [后端双路径架构](后端双路径架构.md) 决策节）。两套工具面互不出现在对方场景。本节记其权威 schema。
