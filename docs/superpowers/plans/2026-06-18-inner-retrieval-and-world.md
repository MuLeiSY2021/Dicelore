...
**Goal:** 补完 anko_driver 内层地基的最后一块——world 域(散文 doc + 可抽样结构 pool)与 rule 域(版本化只读规则)的读写检索、FTS5+jieba 全文检索(event/world_doc/rule_doc,trigram 零依赖保底)、以及可见性写(show/reveal_once)。
...
- Produces:
  ...
  - `ftsMode(): FtsMode`(env `ANKO_FTS_MODE==="trigram"` → trigram,否则 jieba)
...
- **MCP 工具面(组件2)**:把本层包成 `anko_*` 工具 + Zod schema;`sheet_show`/`world_show`/`reveal_once`/`world_register`/`world_sample`/`*_search` 等的薄包装与入出参 schema → [MCP工具面](../../wiki/04-子系统设计/MCP工具面.md)。
