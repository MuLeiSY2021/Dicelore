...
> **上游依赖**：[总体架构 §2 两层工具 / §3 数据工具 / §3.1 可见性 / §4 裁决 / §4.1 narrate / §6 一轮+三流](../03-架构/总体架构.md)；[技术选型 §1 Zod / §4 stdio](../03-架构/技术选型.md)；[02 §2 四业务域+要点5 / §3 行动层](../01-核心概念/核心概念.md)；状态骰下沉 / narrate 升格 / 可见性等决策见本页「决策与权衡」节；求值与存储语义 → [内层能力库](../04-子系统设计/内层能力库.md)。
...
- **expr 全程是字符串、MCP 不解析**（[内层 §3.1](../04-子系统设计/内层能力库.md)）：凡 `expr` 字段都是 `"1d20 + {张三.力量}"` 这样的串，MCP 原样透传给内层求值器，本层不拆。
- **AI 只给引用、不给真实数值**（铁律，[03 §4](../03-架构/总体架构.md)）：靠 expr 的 `{实体.属性}` 约定 + 内层引擎取真值；**schema 不强卡**（自由串拦不住硬编数字），降级为 L2 教 + L3 账本审计（与状态骰下沉同构，见本页决策节）。
...
- **接口契约（本页定骨架，实现期补全文案）**：
  ...
  - **失败路径信封**：出错回 MCP `isError: true` + `content` 带可执行 message，并附结构化 `{ error: { code, message, hint } }`。`code` 枚举：`EXPR_EVAL`（求值失败）/`NOT_NUMERIC`（该掷/算术却给非数值）/`RANGE_INVALID`（档位重叠或不全覆盖）/`ENTITY_NOT_FOUND`/`BAD_INPUT`（入参 schema 校验失败——ZodError 摊成字段级 `path: 原因`，便于 agent 自纠，**不再笼统 INTERNAL**）等。**触发条件归 [内层能力库](../04-子系统设计/内层能力库.md)**，本页只定信封 + code 枚举 +「message 必可执行」。`reminders` **不**兼任错误通道。
...
承接 [02 §3](../01-核心概念/核心概念.md)、[03 §4](../03-架构/总体架构.md)。独立裁决工具——共性：**随机/取真值全在引擎内执行，AI 给不出也改不了真实结果**（anti-F1/F2），且**各落一条"裁决记录"event 供 L3 比对叙述**。

> **resolver 二轴速查表**（选择者 × 结果形状）：resolver = 把未定局面产出一个「叙述者无法伪造」的结果并推动剧情（落 event / 可能写 sheet）。团本特有的只是参数/数据，不长新工具：
>
...
>
> DC 不是独立 judge，是 contest 一边退化成常数；number 形状＝`sheet_update` 带骰；卡池抽＝`world_sample`。明/暗骰二轴（`_open`/`_hidden`）见下方明/暗 L1 名分流。
...
- **暂存语义**（narrate 升格，见本页决策节）：本调用只把"下轮选项+后果"暂存进会话态，**轮内可反复调用改写、末次为准**；AI 的回合自然结束时，**Stop hook 物化**——落一条选项+后果 event（seq 在玩家 pick 之前 → L3 可验"后果在先"）、由输出层呈现给玩家。
- **玩家选择怎么回收**＝模式/adapter 层（聊天/转轮/投票），**非本页**（[adapter](../04-子系统设计/adapter与L3审计.md)）；本页只负责到"暂存 + 物化信号"。
...
- 引擎掷 `die` → `rangeMap` 命中一档（[内层 §2](../04-子系统设计/内层能力库.md)）。**暴击不是引擎逻辑**，是档位表里的一档（如 `{label:"完美",min:96,max:100}`）。
- `die` 是单骰串、不卷入 expr 文法（只 resolver_contest / sheet_update 用 expr）。
- **三档结果（PbtA 对齐，见本页决策节）**：`bands` 是团本/AI 定义"三档结果"的落点——典型应是**完全成功 / 部分成功（成功但有代价）/ 失败（有后果）**而非二元成败，"零代价完全得手"是窄档、非默认。每档 `consequence` 即该档的 fail-forward 后果（[Skills 包 §3](Skills包.md)）。
...
- 两边都是**完整 expr 串**（骰子+引用+常数）；内层求值器各自取真值+掷+求和→比大小，回**账本**（每项 `rolled/ref/int` 标注，[内层 §3.1](../04-子系统设计/内层能力库.md)）。DC 不是独立 judge，是一边为常数。
...
- **阻塞语义**（明骰设计 §3）：handler ① 持久化 `pending_roll`（规格 `die`/`bands`，**无结果**，[内层能力库](../04-子系统设计/内层能力库.md)）→ ② 经后端通知前端「待掷」→ ③ `await awaitPlayerRoll(eventId)` → ④ 玩家点击 → ⑤ core `commitPendingRoll` 此刻掷 + 写 `kind=verdict` event（含点数、命中档、`visible=1`）→ ⑥ **回合内返回结果给 GM**。`awaiting:"player_roll"` 标记本结果经玩家闸控产出。
...
> ⚠️ **2026-07-10 已删**（a-prime §4）：裸 `sheet_get`/`sheet_list` 由类型化读 `player_card`/`npc_list`/`world_state` 取代（kind 由工具名携带）。下方 schema 保留作演进备查；类型化工具见本页决策节「2026-07-10 工具面变更」块。
...
//   prefix 走前缀扫：`张三.`取整卡、`张三.库存:`取整库存（[内层 §4.1](../04-子系统设计/内层能力库.md)）。不用 `_like`（泄漏 SQL）。
...
// in —— 一次 entity 作用域批量写，整批一个事务（原子）
{
  ...
  mutations: z.array(z.object({
    ...
    op:   z.enum(["+", "-", "="]),
    expr: z.string(),                         // 值表达式 / 词条字面量；随 op 多态（[内层 §3.2](../04-子系统设计/内层能力库.md)）
...
// out —— 账本，每项标 rolled|set，喂 L3
{
  ...
  fired_watchers?: [{ watcher_id, payload, condition }],  // 本次写就地触发的 watcher（watcher 机制见内层 §4.2）→ AI 当轮即时反应
  event_id,                                   // 落一条 kind=mutation event（每个触发的 watcher 另落一条 kind=watcher_fired）
}
```

- **状态骰已下沉**（见本页决策节）：带骰项（`HP-2d6`）引擎内掷、AI 给不出真值；纯赋值/集合增减同批，**整批原子**，非数值算术报错+整批回滚。
...
event_append: {
  ...
  visible: z.union([z.literal(0),z.literal(1)]).optional(),  // 省略＝按 kind 默认（[内层 §4.2](../04-子系统设计/内层能力库.md)）
} → { event_id }

event_recall: { query: z.string(), k: z.number().default(8), kind?: ... } → { events: [...] }
//   FTS5(jieba) 召回（[内层 §5](../04-子系统设计/内层能力库.md)）；AI 自读，不靠高亮片段。

watcher_set: {                                // 命名 dicelore_watcher_set（独立名；泛化并取代旧 timer_set，见内层 §4.2 决策）
...
//   edge-triggered（跨越沿触发、条件解除才 re-arm），v1 无显式 cooldown。详见 [内层 §4.2](../04-子系统设计/内层能力库.md)。
...
//   是「未结张力看板」（[backlog-core 主题A](../../../docs/dev/plan/backlog-core.md)）的第一块拼图——仅覆盖 watcher 一处，note/暗格的聚合视图仍待设计周期。
```
...
- **AI 对 rule 只读**（反讨好红线，[02 §2](../01-核心概念/核心概念.md)）；无写工具。被动召回（hook 注入）属 [跨agent §3](../03-架构/跨agent与适配层.md)。

---

## 3. 可见性工具 schema（`sheet_show` / `world_show` / `reveal_once`）

承接 [03 §3.1](../03-架构/总体架构.md)、可见性模型决策（见内层能力库决策节）。GM 用这三个工具控制"玩家能看到什么"；输出层据 `visible` 过滤渲染。
...
承接 [03 §4.1](../03-架构/总体架构.md)、本页决策节 narrate 升格：散文 stream、轮内可多次、**非终结步骤**。
...
- **无 `game_time`**（time 不升格，见内层能力库决策节）；不带"本回合裁决引用"（L3 靠"一个 agent 回合"窗口机械圈定，[内层 §4.2](../04-子系统设计/内层能力库.md)）。
- `narrate` 只**承接 + 记录**散文，不生成叙事；机械结果由输出层渲染（流②），AI 不在 `text` 里吐数值菜单。
- **降级形态**（chat 原生 talk + 自动捕获写 event）→ [adapter](../04-子系统设计/adapter与L3审计.md)。
...
> **回滚 / 快照不在本工具面**（快照决策见 [内层 §4.5](../04-子系统设计/内层能力库.md)、[adapter §8](../04-子系统设计/adapter与L3审计.md)）：撤回 / branch / swipe 是**玩家元动作、非游戏动作**——快照由 Stop hook 写、回滚由 UserPromptSubmit hook auto-sync（兜底 CLI），**不给 AI 工具**（防 AI 把回滚当叙事手段滥用）。故上表无 `rollback` / `checkpoint` 类工具。
...
> **本节职责**：记「团本声明 / 标准库声明 编译出的业务工具如何并入本运行时工具面」的接缝与现状。设计意图见 [团本构建工具链 §7](../04-子系统设计/团本构建工具链.md)（DT-1~9，原声明式工具生成层 spec 已沉该节）、引擎模块同节；本节只记**工具面侧的并入机制 + 实现现状**（单源不重复）。

§1–§7.1 的工具是**框架硬编码**的运行时工具面（`mcp/tools.ts` 的 `TOOLS`：resolver/sheet/event/world/io 五族；运行时 mcp 工具面在 `harness/src/dicegm/mcp/**`，toolgen 编译引擎纯件在 `backend/src/toolgen/**`，见后端双路径架构决策节）。[团本构建工具链 §7](../04-子系统设计/团本构建工具链.md) 定「生成工具并入同一工具面」——团本 `tools:` 声明 + 框架标准库声明经 `toolgen.compileTool` 编译后，**与硬编码工具并列注册进同一 `createMcpServer`**，GM 侧无感（同 `dicelore_` 前缀，类别走 MCP annotation `source:"generated"`）。
...
| **import 装载** | `backend/src/catalog/import.ts` + manifest `tools:` 段 | import 时读团本 `tools:`/`include:`、经**共享 validator**（[团本构建工具链 §7 双校验](../04-子系统设计/团本构建工具链.md)）重验、`compileTool`，交给会话级 MCP server |

**实现现状（2026-07-09 核对）✅ 框架标准库 + 团本侧均已接**：接缝①②③均落地——`backend/src/toolgen/toToolDef.ts` `toolgenToToolDef(decl)` 适配 `compileTool` 产物为 `ToolDef`（zod schema、出参包 `{result}`、读 `readOnlyHint=true`）；`createMcpServer(db, deps, extraTools?)` 加可选 `extraTools` 入口（DT-9 守约：现有工具零改动），`wrapToolForTest` 同步并入。框架标准库叙事工具（`backend/src/stdlib/narration.ts` 八工具）经 `createMcpServer(db, {}, narrationStdlibTools())` 注册、dogfooding 集成测试端到端验证（落库 + 承重墙不破 + 坏声明编译期拒）。接缝③团本 `tools:` 段 import 装载亦已落（`catalog/import.ts` 回传 toolDefs + `build/pack/validate.ts` Rule 8 复用 `compileTool` 校验，2026-06-26 ✅）。详 [backlog-core 主题A′②③](../../../docs/dev/plan/backlog-core.md)。

---

## in-process 挂载工厂 + onCanonWrite 写后接缝（组件7 实时引擎面，2026-06-21）

为让 [组件7 后端](../04-子系统设计/玩家客户端.md) 把 dicelore MCP **in-process** 挂进 Agent SDK（`mcpServers:{dicelore:{type:"sdk",instance}}`），运行时工具面加一个 **additive 工厂**（不改任何工具行为、`main.ts` stdio 路径不变）：
...
- **后端侧映射**：onCanonWrite → `presentation_delta`（普通写，frontend refetch 对账）或 `roll_committed`（`resolve_*_open` 明骰 verdict）。详见 [玩家客户端 §9.2](../04-子系统设计/玩家客户端.md) / [接口页 §4/§5](../04-子系统设计/玩家客户端-接口.md)。

---

## 8. 构建期工具面（`dicelore_build_*`，组件5）

> **本页主体（§1–§7）是运行时工具面**（GM 跑团：resolver / sheet / world / narrate……）。**构建期工具面另属一套**——作者侧、构建期专属，挂在 `LoreSession` 的 in-process 构建 MCP（[团本构建工具链 §6](../04-子系统设计/团本构建工具链.md)、[后端双路径架构 §5](../04-子系统设计/后端双路径架构.md)），与运行时 `TOOLS` **编译期不交叉**（构建期工具面隔离，见 [团本构建工具链](../04-子系统设计/团本构建工具链.md) / [后端双路径架构](../04-子系统设计/后端双路径架构.md) 决策节）。两套工具面互不出现在对方场景。本节记其权威 schema。
...
### 8.2 front md 正典 + prologue 必备（本轮新增 2，见团本构建工具链决策节）
...
| `add_pool` | `{ pool, rows[] }` | 追加随机池行（pool） |
...
> 本节折叠落在「MCP 工具面」这一层（工具 schema / 分流 / 工厂接缝）的关键设计决策与被否方案——**精炼重述,本节即权威(归档已撤)**。同一条决策横切多页时，本节只讲与本层（工具面形状）相关的面。

<details>
<summary>narrate 升格散文 stream + 一轮范式 + 输出层三流</summary>

- **决策（工具面相关面）**：`narrate` = 散文 stream、**非终结步骤**，轮内可多次、任意顺序调用（落点 = §4）；`resolve_choice` = **暂存**下轮选项 + 后果（轮内反复改写、末次为准），回合末经 Stop hook 物化（落点 = §1.1）。**一轮 = agent 一个自然回合**，轮内任何 MCP 调用可重复任意次（落点 = §0 一轮时序）。
- **三流分工**：① narrate 散文 → 对话 + AI 上下文；② 输出层渲染 → 玩家（不进 AI 上下文）；③ resolver / `sheet_update` 结构化结果 → 只回 AI（最小 token，本页 schema 即流③形状）。**AI 不吐数值菜单、只 narrate 色彩**。
- **新硬规矩**：非终局轮回合末必须留有暂存 `resolve_choice`，否则违规；**唯二终局出口** `game_end` / `you_death`（落点 = §6）。
- **被否**：narrate 作单次终结步骤（与作者式多次穿插的真实创作流不符、且逼 AI 吐状态菜单费 token）。

</details>

<details>
<summary>明暗骰工具分流：<code>_hidden</code> / <code>_open</code> 拆名、不加布尔参</summary>

- **决策（工具面相关面）**：掷骰类 resolver 按「掷骰动作归谁」**拆成不同工具名**——**不给 `resolve_*` 加 `gated`/`visible` 布尔参**，而是 `resolve_outcome` / `resolve_contest` 各拆 `_hidden`（暗、引擎自动掷）/ `_open`（明、玩家闸控掷、阻塞式）。逼 GM 在调用点显式回答「这一掷是玩家的还是 GM 的」、不留布尔默认值偷渡。落点 = §1.2–§1.5。
- **重命名**：现有两工具重命名加 `_hidden`（pre-1.0 为消歧值得改，描述里讲明「引擎自动掷、非结果隐藏」）；`resolve_choice` 不动（与明骰同属「玩家面向交互式 resolver」族，一个选、一个掷）。
- **明骰 = 阻塞式 MCP 调用**：`resolve_*_open` 工具调用本身阻塞，结果作工具返回值在同一 GM 回合内回给 GM，不跨回合、不经 Stop 物化；裸 CC 无可阻塞前端 → 当场立即掷、直接返回。
- **anti-F1 边界**：点数恒由 core 在玩家点击时计算；前端只触发 + 播动画 + 成败高亮，客户端伪造不了真值。
- **被否（本层相关面）**：① `resolve_*` 加 `gated`/`visible` 布尔参（消歧靠 L1 工具名分流）；② 明骰走 Stop 物化跨回合（破「回合内见证成败」、丢阻塞返回这一拍）。

</details>

<details>
<summary>createMcpServer 工厂 + onCanonWrite 写后接缝</summary>

- **决策**：core 加 additive 工厂 `createMcpServer(db, deps)`（抽 `main.ts` 的 registerTool 循环；stdio 路径行为不变），在工具处理器**外层包**——工具写规范态成功后回调 `deps.onCanonWrite(evt)`，**不进 `runTool`**（引擎纯逻辑零改动、按实例注入、多 session 安全）。落点 = §7.2 接缝表、「in-process 挂载工厂」节。
- **in-process 挂载**（非 stdio 子进程）为组件7 默认：企业多 session 并发下 stdio 子进程开销线性涨，in-process 单 session 开销低一个量级、工具调用无 IPC 跳；扩展靠多 orchestrator 实例分片 session。
- **后端侧映射**：onCanonWrite → `presentation_delta`（普通写，frontend refetch 对账）/ `roll_committed`（`resolve_*_open` 明骰 verdict）；`deps.rollGate` 经工厂接既有模块级 `setRollGate`（单人；多人 per-instance 化为未来）。
- **被否**：① stdio 默认挂载（多 session 子进程 sprawl、明骰跨进程 gate 须 hack）；② onCanonWrite 写进 `runTool`（破「不改引擎」边界；外层包同样拿得到出参）；③ orchestrator 直接耦合 Agent SDK 不抽 GmDriver（测试须 mock SDK 内部、脆、烧 token）。

</details>

<details>
<summary>2026-07-10 工具面变更：state 类型化 + relation/记忆工具 + 删裸 sheet_get/list + resolve hidden 参 + 明骰 band plan/narration</summary>

- **state 类型化读写**（a-prime §4）：加 `player_card`/`npc_list`/`world_state`（读）+ `player_update`/`npc_update`/`world_update`（写）——**kind 由工具名携带**、查/写对应 kind 视图，全声明式（ToolDecl→toolgen，守 DT-9）。**删裸 `sheet_get`/`sheet_list`**（§2.1 已废·替代品就位）；**保留 `sheet_update`** 作即兴兜底（写 kind=world）+ **保留 `sheet_show`**（kind 无关可见性工具）。TOOLS 21→19。
- **relation 工具**（a-prime §5）：`relation_set(subject,object,dim,value)`（写 rel_object/rel_dim 非空 state 行）/ `relation_query(subject?|object?,dim?,op?,value?)`（查"X 的关系"/"谁对 Y 敌意≥N"），声明式独立工具。
- **记忆工具**（a-prime §6）：`mark_moment(seq)`（标 log is_moment=1）/ `history_compact(seq_from,seq_to)`（agent 读一段 log 写一条 history 摘要·GM 手动调）/ `recall(query)`（先查 history+moment 兜底 FTS log）。**caveat**：recall 用 LIKE、未转义 `%`/`_`。
- **resolve 加 `hidden?` 入参**（hidden-roll §一）：`resolve_outcome`/`resolve_contest` 加 `hidden?:boolean`（默认 false）。`hidden=true` 走**立即掷**路径（不 stage pendingRoll、不等玩家）、结果 event `visible=0`（stream 照发）、发 WS `hidden_roll`（带完整结果）、不发 roll_staged/roll_committed。pendingRoll.shape 不扩 hidden 维度（暗骰不走 pendingRoll·语义错位）。
- **明骰 band 加 `plan`+`narration` 双必填字段**（rollband §一）：`resolve_*_open`/`roll_staged` 入参 bands 每档要求 `plan`（AI 真实计划·驱动机械·可含暗值·骰前锁定）+ `narration`（玩家可见叙述·可留白），缺失校验失败。两字段随 `roll_staged` 全量下发（不剥），显隐交前端 spoiler 档；掷骰后据命中档 `plan` 驱动机械（sheet_update/Front 推进·一物两用）。详见 [玩家客户端-接口 D8/D9](../04-子系统设计/玩家客户端-接口.md)。
- **被否**：resolve 加布尔参承载暗骰（延续 `_hidden`/`_open` 名分流精神，但暗骰经 `hidden` 参而非新工具——因暗骰是「立即掷」路径开关、非「谁掷」的语义分流）；band 单 narration 字段（泄露暗值或丢机械真相）。
...
- 表 schema、FTS5、`expr` 文法与求值、`visible` 列存储/强制隐藏标记/`reveal_once`=reveal 的写入语义 → [内层能力库](../04-子系统设计/内层能力库.md)
- 错误的**触发条件**（求值失败、`rangeMap` 校验、非数值算术）与 **CHARACTER_LIMIT 截断的 enforcement** → [内层能力库](../04-子系统设计/内层能力库.md)（本页只定错误信封 + code 枚举 + 截断字段约定）
- 教条内容（别软着陆怎么写）、补刀**措辞**与触发表条目、工具选择决策树 skill → [Skills 包](Skills包.md)（本页只定补刀的**结构挂载点**）
- 各 agent 的工具注册、`narrate` 降级、Stop hook（物化 choice + L3 审计）、被动 rule 召回 hook、watcher 触发 payload 怎么经 `sheet_update` 出参回 AI、玩家输入捕获 → [adapter 与 L3 审计](../04-子系统设计/adapter与L3审计.md) / [跨agent与适配层](../03-架构/跨agent与适配层.md)
