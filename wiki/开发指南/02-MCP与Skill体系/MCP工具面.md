...
> **本页主体（§1–§7）是运行时工具面**（GM 跑团：resolver / sheet / world / narrate……）。**构建期工具面另属一套**——作者侧、构建期专属，挂在 `LoreSession` 的 in-process 构建 MCP（[团本构建工具链 §6](../04-子系统设计/团本构建工具链.md)、[后端双路径架构 §5](后端双路径架构.md)），与运行时 `TOOLS` **编译期不交叉**（构建期工具面隔离，见 [团本构建工具链](../04-子系统设计/团本构建工具链.md) / [后端双路径架构](后端双路径架构.md) 决策节）。两套工具面互不出现在对方场景。本节记其权威 schema。
...
- **明骰 band 加 `plan`+`narration` 双必填字段**（rollband §一）：`resolve_*_open`/`roll_staged` 入参 bands 每档要求 `plan`（AI 真实计划·驱动机械·可含暗值·骰前锁定）+ `narration`（玩家可见叙述·可留白），缺失校验失败。两字段随 `roll_staged` 全量下发（不剥），显隐交前端 spoiler 档；掷骰后据命中档 `plan` 驱动机械（sheet_update/Front 推进·一物两用）。详见 [玩家客户端-接口 D8/D9](玩家客户端-接口.md)。
