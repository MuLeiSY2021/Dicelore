                ...
                <div key={ri}>
                  ...
                  {r.usage && (
                    <div className="turn-usage" data-testid="play-turn-usage"
                      ...
                      ⟨<span className="tu-model">{r.model ?? usage?.model ?? ""}</span> · <span className="up">↑{fmtTokens(r.usage.inputTokens)}</span> <span className="down">↓{fmtTokens(r.usage.outputTokens)}</span> tok · ≈${estimateCostUsd(r.usage, r.model ?? usage?.model).toFixed(3)}⟩
                    </div>
                  )}
                </div>
              ))}
