...
> 为什么存在、端口表面怎么圈定 → 见 [总体架构「决策与权衡」节](../../wiki/开发指南/03-架构/总体架构.md) ADR-0028 原决策①②③。
...
依赖：叶包 `@dicelore/dice`（域类型引 `Rng`）+ `better-sqlite3`（`DB` 句柄类型，type-only）+ `zod`（`ToolDef.inputSchema` schema 类型）。`backend` 实现接口、`harness` 经注入的接口调存储——见 [总体架构「决策与权衡」节 ADR-0028 决策④ 组合根与生命周期](../../wiki/开发指南/03-架构/总体架构.md)。
