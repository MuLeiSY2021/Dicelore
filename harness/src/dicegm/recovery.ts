...
export function replayNarration(host: { backend: SessionBackend; hub: WsHub; sessionId: string }, since: number): number {
  const rows = host.backend.logSince(since).filter((r) => r.kind === "narrate" && r.visible === 1);
  for (const r of rows) {
