...
//
// ⚠️ 关键设计点(RT-5):lore 构建是 **REST only**。POST /sessions/loregm/:id/messages 把构建 GM 驱动到
...
//   检视类:get_draft(看未 commit 的 Draft 当前态——靠 GET /sessions/loregm/:id/draft,见下注)、
...
// 不回传散文。会话须先经 open_build_session 显式建(session-surface-flatten C2 移除懒建)。
...
export async function doSendToBuilder(sid: string, text: string): Promise<{ turnId: string; error?: { message: string; code?: string } }> {
  return (await post(`/sessions/loregm/${enc(sid)}/messages`, { text })) as { turnId: string; error?: { message: string; code?: string } };
}
// 起一个构建会话(session-surface-flatten §三):显式 POST /sessions/loregm {name?} → 201 {sessionId,kind}。
// 服务端生成 sessionId、建 Draft + 构建 MCP(空 Draft 起步)。name = 在造的团本名(→ 后端 UUIDv5 身份)。
// 取代旧「本地随机 id + 首访懒建」——懒建已移除,须显式建后才能 send/get_draft。
export async function doOpenBuildSession(name?: string): Promise<string> {
  const r = (await post(`/sessions/loregm`, name === undefined ? {} : { name })) as { sessionId: string; kind: string };
  return r.sessionId;
}
// 检视:看未 commit 的 Draft 当前态(构建中途产物)。返回 { files(将提交的包文件), snapshot(分域结构化) }。
// 这是判断「构建 GM 干了什么 / 进度」的主检视面——因构建 GM 响应不经 REST 返回。
export const doGetDraft = (sid: string) => jfetch(`/sessions/loregm/${enc(sid)}/draft`);
...
export async function doPutMaterial(sid: string, filename: string, localPath: string): Promise<{ path: string; bytes: number }> {
  ...
  const r = await fetch(`${PLAY_URL()}/sessions/loregm/${enc(sid)}/materials?filename=${encodeURIComponent(filename)}`, {
    method: "POST",
...
async function main() {
  ...
  server.tool("open_build_session", "显式建一个构建会话(POST /sessions/loregm)。服务端生成 sessionId 并建空 Draft;name=在造团本名(可省)。返回 sessionId,作者经它驱动构建 GM 造团本。", { name: z.string().optional() }, rw, async ({ name }) => json(await doOpenBuildSession(name)));
  server.tool("send_to_builder", "作者自然语言指令驱动构建 GM 一轮(改 in-memory Draft)。会话须先 open_build_session。REST only:只返回 {turnId},不回传 GM 散文——靠 get_draft/list_catalog 检视产物。", { sessionId: z.string(), text: z.string() }, rw, async ({ sessionId, text }) => json(await doSendToBuilder(sessionId, text)));
