...
description: Dicelore 路线图的「编排执行」上层 skill。当用户要「一路推进路线图 / 尽量别问我 / 把这几批做完」时用。核心:编排者(主 agent)只做分析、分解、对答、检查、合并,**永远派 subagent 执行、自己不下场写代码**——哪怕分析完是纯串行依赖链,也开 subagent 一个接一个串行执行。在 [autonomous-delivery-loop](../autonomous-delivery-loop/SKILL.md)（一条线内部怎么干）之上加:可逆性判据(决定哪些卡点自己拍、哪些攒着批量问用户)、把路线图递归分解成沿文件缝切的原子节点(落 nodes.jsonl DAG)、按依赖就绪派发 subagent(出生即就绪不 await)、subagent 自主交付一条线(自写 spec/plan + push 自己分支 + 开 PR + 缺依赖就问不自造)、编排者检查 PR(CI 绿+diff+契约)通过才合并并释放下游、subagent 解不了的卡点先回编排者(能定就定、定不了才问用户)。
---

# 并发路线图交付（parallel-roadmap-delivery）

[autonomous-delivery-loop](../autonomous-delivery-loop/SKILL.md) 定义**一条线**内部怎么干（差距分析 → 设计 → 实现 → 验收 → 沉淀）。本 skill 是它的**编排执行上层**：当用户说「别老问我、一路推下去」时，编排者（主 agent）**只做分析、分解、对答、检查、合并，把所有写代码的活都派给 subagent，自己永远不下场**。
...
- 📎 一条线**内部**怎么干（设计→实现→验收→沉淀）不在本 skill——那是 [autonomous-delivery-loop](../autonomous-delivery-loop/SKILL.md)，每个 subagent 自己跑它。本 skill 只管「怎么把活拆成线、派给谁、卡点怎么不打扰用户、PR 怎么检查合并」。
...
本 skill 只定「怎么编排执行」——分解、派发、对答、检查、合并，以及卡点怎么不打扰用户。**一条线内部的流程**（差距分析 → 落 backlog → 设计 → 实现 → 验收 → 沉淀）权威在 [autonomous-delivery-loop](../autonomous-delivery-loop/SKILL.md)，这里不复述。**每个 subagent 在自己那条线里跑的就是那一套**——autonomous-delivery-loop 不再是「编排者单线时的退路」，而是「被 subagent 引用的、一条线内部怎么干」。改单线流程改那个 skill，别在这另起一套。
