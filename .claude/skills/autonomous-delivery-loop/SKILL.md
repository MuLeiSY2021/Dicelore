...
> **多条无依赖线要并行推、且要把决策卡点压到最少** → 上层用 [parallel-roadmap-delivery](../parallel-roadmap-delivery/SKILL.md)（决策账本 + 波次 fan-out + SendMessage 续接 + CI/PR 门禁）；**每条并行线内部仍走本 skill 的 a→g**。单线或纯串行依赖链 → 直接用本 skill，别套并发开销。
