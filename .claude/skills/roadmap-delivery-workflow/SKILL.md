...
阶段1（交互·主 agent，不进 Workflow）
  ...
  按原子需求切 DAG → delivery_dag.md（**一份裁决炸成 1..N 个原子需求节点，非 1 裁决=1 节点**；全图依赖+谁太大要拆+分几波+热点文件冲突）
...
阶段3（交互·主 agent）
  逐节点检查：typecheck/test 绿 + diff 审 + 契约符合
     通过 → 集成进本地 main（重叠冲突主 agent 当场解）→ 沉 wiki → wave_N_summary.md 记复盘
...
- **生命周期**：用户勾批准 → 进波交付 → 完成后把最终设计沉淀进 wiki（设计页）→ **删裁决文件**（阶段3 收尾，过渡稿不长存）。
...
分解先落成本轮目录的 **`delivery_dag.md`**（设计稿：全图依赖 + 谁太大要拆 + 分几波 + 热点文件冲突表）。发某一波时，把该波 roster **冻结**成 **`wave_N_nodes.jsonl`**（一行一个**需求**，发波即冻结、不回头改）：
...
**合并后收尾**（主 agent 的活，不下放）：沉淀 wiki（现状 🚧→✅、关 backlog 条目、勾路线图、spec/plan 整套落地才清）→ **确认设计结论已进 wiki 后，删掉该需求的裁决文件**（`docs/wiki/设计/05-现状与计划/裁决记录/<需求>.md`，过渡稿不长存，留着是双写漂移源）+ 路线图该项的「已裁决」链接随之失效转「已归档」。**agent 调查中的现状结论也要沉 wiki**，别只留在返回里（教训 `[调查要沉淀进wiki]`）。
