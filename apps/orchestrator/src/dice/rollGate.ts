...
import { getPendingRoll, commitPendingRoll, type DB, type RollGate, type PendingRollRow, type RollResult } from "@dicelore/core";
...
export class PlayerRollGate {
  ...
  resolveRoll(eventId: number): boolean {
    ...
    const pr = getPendingRoll(this.db, eventId);
    if (!pr) return false; // 未知 eventId(或非本会话)——端点据此回 409 no_pending_roll，语义正确。
    const r = commitPendingRoll(this.db, eventId);
