...
import { stagePendingRoll, getRollGate, metaSet, type DB } from "@dicelore/core";
...
export class FakeDiceGm implements Agent {
  ...
  //  · new FakeDiceGm({ canon, db })         — 教练档：驱动 roll/choice/game_end 五条主线。
  constructor(arg: Script | { canon: CanonScript; db: DB }) {
        ...
        switch (a.type) {
          ...
          case "choice":
            ...
            db.prepare("INSERT INTO log (content, kind, data_json, visible) VALUES (?, 'choice', ?, 1)")
              .run(a.prompt, JSON.stringify({ prompt: a.prompt, options: a.options }));
            break;
          case "gameEnd": {
            // 落 game_end note + 写 ended 元。reason/outcome 由 DiceSession.enrich 从 session_meta 取出。
            const info = db.prepare("INSERT INTO log (content, kind, data_json, visible) VALUES (?, 'note', ?, 0)")
              .run(null, JSON.stringify({ reason: a.reason, outcome: a.outcome }));
            metaSet(db, "ended", JSON.stringify({ reason: a.reason, outcome: a.outcome, seq: Number(info.lastInsertRowid) }));
            break;
          }
          default:
            break;
