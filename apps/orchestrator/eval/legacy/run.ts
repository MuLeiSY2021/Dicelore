...
//
// 灌种子 + 建临时 db 已抽到 src/eval/scenario.ts 的 prepareSessionDb(与 orchestrator harness 共用)，
// 本文件只保留 run.ts 专属的「init 临时项目 + 重写 .mcp.json 指本地 tsx + 打印手动指引」。
import { writeFileSync, rmSync, existsSync, mkdtempSync } from "node:fs";
import { tmpdir } from "node:os";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { prepareSessionDb } from "../src/eval/scenario.js";
...
// 灌种子 + 建临时 db(env 在 prepareSessionDb 内部设好)。与 orchestrator harness 共用同一场景准备逻辑。
const { dbPath, scenario, sessionName } = await prepareSessionDb(scenarioId, { baseline, sessionsDir });
...
console.log(`     - 手动: cd ${projectDir} && DICELORE_SESSIONS_DIR=${sessionsDir} DICELORE_SESSION=${sessionName} claude  (逐条贴上面输入)`);
