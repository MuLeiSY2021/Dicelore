...
test("造示例团本 → 开局 import → 跑一回合 → 看到导入态与叙事", async ({ page }) => {
  ...
  await expect(page.getByText("旅人")).toBeVisible({ timeout: 10_000 });
  await expect(page.getByText("12")).toBeVisible();

  // 3. 发一句话 → FAKE GM 回叙事(WS narration_commit)
  const input = page.locator("input").first();
  await input.fill("我推门进去");
  await input.press("Enter");
  await expect(page.getByText(/门吱呀一声开了|你说/)).toBeVisible({ timeout: 10_000 });
});
